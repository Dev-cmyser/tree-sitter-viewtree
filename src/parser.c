/* Automatically @generated by tree-sitter v0.25.10 */

#include "tree_sitter/parser.h"

#if defined(__GNUC__) || defined(__clang__)
#pragma GCC diagnostic ignored "-Wmissing-field-initializers"
#endif

#define LANGUAGE_VERSION 15
#define STATE_COUNT 67
#define LARGE_STATE_COUNT 10
#define SYMBOL_COUNT 51
#define ALIAS_COUNT 0
#define TOKEN_COUNT 27
#define EXTERNAL_TOKEN_COUNT 3
#define FIELD_COUNT 0
#define MAX_ALIAS_SEQUENCE_LENGTH 4
#define MAX_RESERVED_WORD_SET_SIZE 0
#define PRODUCTION_ID_COUNT 1
#define SUPERTYPE_COUNT 0

enum ts_symbol_identifiers {
  aux_sym_comment_content_token1 = 1,
  anon_sym_SPACE = 2,
  sym_component_name = 3,
  anon_sym_LT_EQ_GT = 4,
  anon_sym_LT_EQ = 5,
  anon_sym_EQ_GT = 6,
  sym_comment_marker = 7,
  anon_sym_SLASH = 8,
  anon_sym_STAR = 9,
  anon_sym_CARET = 10,
  anon_sym_AT = 11,
  aux_sym_localized_text_token1 = 12,
  anon_sym_QMARK = 13,
  anon_sym_BANG = 14,
  sym_identifier = 15,
  aux_sym_number_token1 = 16,
  aux_sym_number_token2 = 17,
  anon_sym_NaN = 18,
  anon_sym_Infinity = 19,
  anon_sym_DASHInfinity = 20,
  anon_sym_true = 21,
  anon_sym_false = 22,
  sym_null = 23,
  sym__indent = 24,
  sym__dedent = 25,
  sym__newline = 26,
  sym_source_file = 27,
  sym_line = 28,
  sym_commented_line = 29,
  sym_comment_content = 30,
  sym_commented_children = 31,
  sym_commented_child_line = 32,
  sym_normal_line = 33,
  sym_children = 34,
  sym_token_chain = 35,
  sym__token = 36,
  sym_binding_operator = 37,
  sym_typed_list = 38,
  sym_typed_dict = 39,
  sym_list_marker = 40,
  sym_dict_marker = 41,
  sym_localized_string = 42,
  sym_localized_text = 43,
  sym_string_literal = 44,
  sym_identifier_with_modifier = 45,
  sym_property_modifier = 46,
  sym_number = 47,
  sym_boolean = 48,
  aux_sym_source_file_repeat1 = 49,
  aux_sym_commented_children_repeat1 = 50,
};

static const char * const ts_symbol_names[] = {
  [ts_builtin_sym_end] = "end",
  [aux_sym_comment_content_token1] = "comment_content_token1",
  [anon_sym_SPACE] = " ",
  [sym_component_name] = "component_name",
  [anon_sym_LT_EQ_GT] = "<=>",
  [anon_sym_LT_EQ] = "<=",
  [anon_sym_EQ_GT] = "=>",
  [sym_comment_marker] = "comment_marker",
  [anon_sym_SLASH] = "/",
  [anon_sym_STAR] = "*",
  [anon_sym_CARET] = "^",
  [anon_sym_AT] = "@",
  [aux_sym_localized_text_token1] = "localized_text_token1",
  [anon_sym_QMARK] = "\?",
  [anon_sym_BANG] = "!",
  [sym_identifier] = "identifier",
  [aux_sym_number_token1] = "number_token1",
  [aux_sym_number_token2] = "number_token2",
  [anon_sym_NaN] = "NaN",
  [anon_sym_Infinity] = "Infinity",
  [anon_sym_DASHInfinity] = "-Infinity",
  [anon_sym_true] = "true",
  [anon_sym_false] = "false",
  [sym_null] = "null",
  [sym__indent] = "_indent",
  [sym__dedent] = "_dedent",
  [sym__newline] = "_newline",
  [sym_source_file] = "source_file",
  [sym_line] = "line",
  [sym_commented_line] = "commented_line",
  [sym_comment_content] = "comment_content",
  [sym_commented_children] = "commented_children",
  [sym_commented_child_line] = "commented_child_line",
  [sym_normal_line] = "normal_line",
  [sym_children] = "children",
  [sym_token_chain] = "token_chain",
  [sym__token] = "_token",
  [sym_binding_operator] = "binding_operator",
  [sym_typed_list] = "typed_list",
  [sym_typed_dict] = "typed_dict",
  [sym_list_marker] = "list_marker",
  [sym_dict_marker] = "dict_marker",
  [sym_localized_string] = "localized_string",
  [sym_localized_text] = "localized_text",
  [sym_string_literal] = "string_literal",
  [sym_identifier_with_modifier] = "identifier_with_modifier",
  [sym_property_modifier] = "property_modifier",
  [sym_number] = "number",
  [sym_boolean] = "boolean",
  [aux_sym_source_file_repeat1] = "source_file_repeat1",
  [aux_sym_commented_children_repeat1] = "commented_children_repeat1",
};

static const TSSymbol ts_symbol_map[] = {
  [ts_builtin_sym_end] = ts_builtin_sym_end,
  [aux_sym_comment_content_token1] = aux_sym_comment_content_token1,
  [anon_sym_SPACE] = anon_sym_SPACE,
  [sym_component_name] = sym_component_name,
  [anon_sym_LT_EQ_GT] = anon_sym_LT_EQ_GT,
  [anon_sym_LT_EQ] = anon_sym_LT_EQ,
  [anon_sym_EQ_GT] = anon_sym_EQ_GT,
  [sym_comment_marker] = sym_comment_marker,
  [anon_sym_SLASH] = anon_sym_SLASH,
  [anon_sym_STAR] = anon_sym_STAR,
  [anon_sym_CARET] = anon_sym_CARET,
  [anon_sym_AT] = anon_sym_AT,
  [aux_sym_localized_text_token1] = aux_sym_localized_text_token1,
  [anon_sym_QMARK] = anon_sym_QMARK,
  [anon_sym_BANG] = anon_sym_BANG,
  [sym_identifier] = sym_identifier,
  [aux_sym_number_token1] = aux_sym_number_token1,
  [aux_sym_number_token2] = aux_sym_number_token2,
  [anon_sym_NaN] = anon_sym_NaN,
  [anon_sym_Infinity] = anon_sym_Infinity,
  [anon_sym_DASHInfinity] = anon_sym_DASHInfinity,
  [anon_sym_true] = anon_sym_true,
  [anon_sym_false] = anon_sym_false,
  [sym_null] = sym_null,
  [sym__indent] = sym__indent,
  [sym__dedent] = sym__dedent,
  [sym__newline] = sym__newline,
  [sym_source_file] = sym_source_file,
  [sym_line] = sym_line,
  [sym_commented_line] = sym_commented_line,
  [sym_comment_content] = sym_comment_content,
  [sym_commented_children] = sym_commented_children,
  [sym_commented_child_line] = sym_commented_child_line,
  [sym_normal_line] = sym_normal_line,
  [sym_children] = sym_children,
  [sym_token_chain] = sym_token_chain,
  [sym__token] = sym__token,
  [sym_binding_operator] = sym_binding_operator,
  [sym_typed_list] = sym_typed_list,
  [sym_typed_dict] = sym_typed_dict,
  [sym_list_marker] = sym_list_marker,
  [sym_dict_marker] = sym_dict_marker,
  [sym_localized_string] = sym_localized_string,
  [sym_localized_text] = sym_localized_text,
  [sym_string_literal] = sym_string_literal,
  [sym_identifier_with_modifier] = sym_identifier_with_modifier,
  [sym_property_modifier] = sym_property_modifier,
  [sym_number] = sym_number,
  [sym_boolean] = sym_boolean,
  [aux_sym_source_file_repeat1] = aux_sym_source_file_repeat1,
  [aux_sym_commented_children_repeat1] = aux_sym_commented_children_repeat1,
};

static const TSSymbolMetadata ts_symbol_metadata[] = {
  [ts_builtin_sym_end] = {
    .visible = false,
    .named = true,
  },
  [aux_sym_comment_content_token1] = {
    .visible = false,
    .named = false,
  },
  [anon_sym_SPACE] = {
    .visible = true,
    .named = false,
  },
  [sym_component_name] = {
    .visible = true,
    .named = true,
  },
  [anon_sym_LT_EQ_GT] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_LT_EQ] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_EQ_GT] = {
    .visible = true,
    .named = false,
  },
  [sym_comment_marker] = {
    .visible = true,
    .named = true,
  },
  [anon_sym_SLASH] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_STAR] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_CARET] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_AT] = {
    .visible = true,
    .named = false,
  },
  [aux_sym_localized_text_token1] = {
    .visible = false,
    .named = false,
  },
  [anon_sym_QMARK] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_BANG] = {
    .visible = true,
    .named = false,
  },
  [sym_identifier] = {
    .visible = true,
    .named = true,
  },
  [aux_sym_number_token1] = {
    .visible = false,
    .named = false,
  },
  [aux_sym_number_token2] = {
    .visible = false,
    .named = false,
  },
  [anon_sym_NaN] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_Infinity] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_DASHInfinity] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_true] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_false] = {
    .visible = true,
    .named = false,
  },
  [sym_null] = {
    .visible = true,
    .named = true,
  },
  [sym__indent] = {
    .visible = false,
    .named = true,
  },
  [sym__dedent] = {
    .visible = false,
    .named = true,
  },
  [sym__newline] = {
    .visible = false,
    .named = true,
  },
  [sym_source_file] = {
    .visible = true,
    .named = true,
  },
  [sym_line] = {
    .visible = true,
    .named = true,
  },
  [sym_commented_line] = {
    .visible = true,
    .named = true,
  },
  [sym_comment_content] = {
    .visible = true,
    .named = true,
  },
  [sym_commented_children] = {
    .visible = true,
    .named = true,
  },
  [sym_commented_child_line] = {
    .visible = true,
    .named = true,
  },
  [sym_normal_line] = {
    .visible = true,
    .named = true,
  },
  [sym_children] = {
    .visible = true,
    .named = true,
  },
  [sym_token_chain] = {
    .visible = true,
    .named = true,
  },
  [sym__token] = {
    .visible = false,
    .named = true,
  },
  [sym_binding_operator] = {
    .visible = true,
    .named = true,
  },
  [sym_typed_list] = {
    .visible = true,
    .named = true,
  },
  [sym_typed_dict] = {
    .visible = true,
    .named = true,
  },
  [sym_list_marker] = {
    .visible = true,
    .named = true,
  },
  [sym_dict_marker] = {
    .visible = true,
    .named = true,
  },
  [sym_localized_string] = {
    .visible = true,
    .named = true,
  },
  [sym_localized_text] = {
    .visible = true,
    .named = true,
  },
  [sym_string_literal] = {
    .visible = true,
    .named = true,
  },
  [sym_identifier_with_modifier] = {
    .visible = true,
    .named = true,
  },
  [sym_property_modifier] = {
    .visible = true,
    .named = true,
  },
  [sym_number] = {
    .visible = true,
    .named = true,
  },
  [sym_boolean] = {
    .visible = true,
    .named = true,
  },
  [aux_sym_source_file_repeat1] = {
    .visible = false,
    .named = false,
  },
  [aux_sym_commented_children_repeat1] = {
    .visible = false,
    .named = false,
  },
};

static const TSSymbol ts_alias_sequences[PRODUCTION_ID_COUNT][MAX_ALIAS_SEQUENCE_LENGTH] = {
  [0] = {0},
};

static const uint16_t ts_non_terminal_alias_map[] = {
  0,
};

static const TSStateId ts_primary_state_ids[STATE_COUNT] = {
  [0] = 0,
  [1] = 1,
  [2] = 2,
  [3] = 3,
  [4] = 4,
  [5] = 2,
  [6] = 4,
  [7] = 7,
  [8] = 7,
  [9] = 9,
  [10] = 10,
  [11] = 11,
  [12] = 12,
  [13] = 11,
  [14] = 12,
  [15] = 10,
  [16] = 16,
  [17] = 17,
  [18] = 18,
  [19] = 19,
  [20] = 16,
  [21] = 21,
  [22] = 22,
  [23] = 18,
  [24] = 17,
  [25] = 19,
  [26] = 22,
  [27] = 21,
  [28] = 28,
  [29] = 29,
  [30] = 30,
  [31] = 31,
  [32] = 32,
  [33] = 31,
  [34] = 31,
  [35] = 35,
  [36] = 35,
  [37] = 37,
  [38] = 35,
  [39] = 39,
  [40] = 40,
  [41] = 41,
  [42] = 42,
  [43] = 43,
  [44] = 44,
  [45] = 42,
  [46] = 21,
  [47] = 47,
  [48] = 48,
  [49] = 49,
  [50] = 50,
  [51] = 51,
  [52] = 52,
  [53] = 53,
  [54] = 54,
  [55] = 55,
  [56] = 56,
  [57] = 57,
  [58] = 58,
  [59] = 59,
  [60] = 60,
  [61] = 61,
  [62] = 58,
  [63] = 63,
  [64] = 61,
  [65] = 65,
  [66] = 66,
};

static const TSCharacterRange sym_identifier_character_set_1[] = {
  {'#', '#'}, {'*', '*'}, {'-', '-'}, {'0', '9'}, {'A', 'Z'}, {'_', '_'}, {'a', 'z'}, {0xa9, 0xaa},
  {0xae, 0xae}, {0xb5, 0xb5}, {0xba, 0xba}, {0xc0, 0xd6}, {0xd8, 0xf6}, {0xf8, 0x2c1}, {0x2c6, 0x2d1}, {0x2e0, 0x2e4},
  {0x2ec, 0x2ec}, {0x2ee, 0x2ee}, {0x370, 0x374}, {0x376, 0x377}, {0x37a, 0x37d}, {0x37f, 0x37f}, {0x386, 0x386}, {0x388, 0x38a},
  {0x38c, 0x38c}, {0x38e, 0x3a1}, {0x3a3, 0x3f5}, {0x3f7, 0x481}, {0x48a, 0x52f}, {0x531, 0x556}, {0x559, 0x559}, {0x560, 0x588},
  {0x5d0, 0x5ea}, {0x5ef, 0x5f2}, {0x620, 0x64a}, {0x66e, 0x66f}, {0x671, 0x6d3}, {0x6d5, 0x6d5}, {0x6e5, 0x6e6}, {0x6ee, 0x6ef},
  {0x6fa, 0x6fc}, {0x6ff, 0x6ff}, {0x710, 0x710}, {0x712, 0x72f}, {0x74d, 0x7a5}, {0x7b1, 0x7b1}, {0x7ca, 0x7ea}, {0x7f4, 0x7f5},
  {0x7fa, 0x7fa}, {0x800, 0x815}, {0x81a, 0x81a}, {0x824, 0x824}, {0x828, 0x828}, {0x840, 0x858}, {0x860, 0x86a}, {0x870, 0x887},
  {0x889, 0x88e}, {0x8a0, 0x8c9}, {0x904, 0x939}, {0x93d, 0x93d}, {0x950, 0x950}, {0x958, 0x961}, {0x971, 0x980}, {0x985, 0x98c},
  {0x98f, 0x990}, {0x993, 0x9a8}, {0x9aa, 0x9b0}, {0x9b2, 0x9b2}, {0x9b6, 0x9b9}, {0x9bd, 0x9bd}, {0x9ce, 0x9ce}, {0x9dc, 0x9dd},
  {0x9df, 0x9e1}, {0x9f0, 0x9f1}, {0x9fc, 0x9fc}, {0xa05, 0xa0a}, {0xa0f, 0xa10}, {0xa13, 0xa28}, {0xa2a, 0xa30}, {0xa32, 0xa33},
  {0xa35, 0xa36}, {0xa38, 0xa39}, {0xa59, 0xa5c}, {0xa5e, 0xa5e}, {0xa72, 0xa74}, {0xa85, 0xa8d}, {0xa8f, 0xa91}, {0xa93, 0xaa8},
  {0xaaa, 0xab0}, {0xab2, 0xab3}, {0xab5, 0xab9}, {0xabd, 0xabd}, {0xad0, 0xad0}, {0xae0, 0xae1}, {0xaf9, 0xaf9}, {0xb05, 0xb0c},
  {0xb0f, 0xb10}, {0xb13, 0xb28}, {0xb2a, 0xb30}, {0xb32, 0xb33}, {0xb35, 0xb39}, {0xb3d, 0xb3d}, {0xb5c, 0xb5d}, {0xb5f, 0xb61},
  {0xb71, 0xb71}, {0xb83, 0xb83}, {0xb85, 0xb8a}, {0xb8e, 0xb90}, {0xb92, 0xb95}, {0xb99, 0xb9a}, {0xb9c, 0xb9c}, {0xb9e, 0xb9f},
  {0xba3, 0xba4}, {0xba8, 0xbaa}, {0xbae, 0xbb9}, {0xbd0, 0xbd0}, {0xc05, 0xc0c}, {0xc0e, 0xc10}, {0xc12, 0xc28}, {0xc2a, 0xc39},
  {0xc3d, 0xc3d}, {0xc58, 0xc5a}, {0xc5d, 0xc5d}, {0xc60, 0xc61}, {0xc80, 0xc80}, {0xc85, 0xc8c}, {0xc8e, 0xc90}, {0xc92, 0xca8},
  {0xcaa, 0xcb3}, {0xcb5, 0xcb9}, {0xcbd, 0xcbd}, {0xcdd, 0xcde}, {0xce0, 0xce1}, {0xcf1, 0xcf2}, {0xd04, 0xd0c}, {0xd0e, 0xd10},
  {0xd12, 0xd3a}, {0xd3d, 0xd3d}, {0xd4e, 0xd4e}, {0xd54, 0xd56}, {0xd5f, 0xd61}, {0xd7a, 0xd7f}, {0xd85, 0xd96}, {0xd9a, 0xdb1},
  {0xdb3, 0xdbb}, {0xdbd, 0xdbd}, {0xdc0, 0xdc6}, {0xe01, 0xe30}, {0xe32, 0xe33}, {0xe40, 0xe46}, {0xe81, 0xe82}, {0xe84, 0xe84},
  {0xe86, 0xe8a}, {0xe8c, 0xea3}, {0xea5, 0xea5}, {0xea7, 0xeb0}, {0xeb2, 0xeb3}, {0xebd, 0xebd}, {0xec0, 0xec4}, {0xec6, 0xec6},
  {0xedc, 0xedf}, {0xf00, 0xf00}, {0xf40, 0xf47}, {0xf49, 0xf6c}, {0xf88, 0xf8c}, {0x1000, 0x102a}, {0x103f, 0x103f}, {0x1050, 0x1055},
  {0x105a, 0x105d}, {0x1061, 0x1061}, {0x1065, 0x1066}, {0x106e, 0x1070}, {0x1075, 0x1081}, {0x108e, 0x108e}, {0x10a0, 0x10c5}, {0x10c7, 0x10c7},
  {0x10cd, 0x10cd}, {0x10d0, 0x10fa}, {0x10fc, 0x1248}, {0x124a, 0x124d}, {0x1250, 0x1256}, {0x1258, 0x1258}, {0x125a, 0x125d}, {0x1260, 0x1288},
  {0x128a, 0x128d}, {0x1290, 0x12b0}, {0x12b2, 0x12b5}, {0x12b8, 0x12be}, {0x12c0, 0x12c0}, {0x12c2, 0x12c5}, {0x12c8, 0x12d6}, {0x12d8, 0x1310},
  {0x1312, 0x1315}, {0x1318, 0x135a}, {0x1380, 0x138f}, {0x13a0, 0x13f5}, {0x13f8, 0x13fd}, {0x1401, 0x166c}, {0x166f, 0x167f}, {0x1681, 0x169a},
  {0x16a0, 0x16ea}, {0x16f1, 0x16f8}, {0x1700, 0x1711}, {0x171f, 0x1731}, {0x1740, 0x1751}, {0x1760, 0x176c}, {0x176e, 0x1770}, {0x1780, 0x17b3},
  {0x17d7, 0x17d7}, {0x17dc, 0x17dc}, {0x1820, 0x1878}, {0x1880, 0x1884}, {0x1887, 0x18a8}, {0x18aa, 0x18aa}, {0x18b0, 0x18f5}, {0x1900, 0x191e},
  {0x1950, 0x196d}, {0x1970, 0x1974}, {0x1980, 0x19ab}, {0x19b0, 0x19c9}, {0x1a00, 0x1a16}, {0x1a20, 0x1a54}, {0x1aa7, 0x1aa7}, {0x1b05, 0x1b33},
  {0x1b45, 0x1b4c}, {0x1b83, 0x1ba0}, {0x1bae, 0x1baf}, {0x1bba, 0x1be5}, {0x1c00, 0x1c23}, {0x1c4d, 0x1c4f}, {0x1c5a, 0x1c7d}, {0x1c80, 0x1c8a},
  {0x1c90, 0x1cba}, {0x1cbd, 0x1cbf}, {0x1ce9, 0x1cec}, {0x1cee, 0x1cf3}, {0x1cf5, 0x1cf6}, {0x1cfa, 0x1cfa}, {0x1d00, 0x1dbf}, {0x1e00, 0x1f15},
  {0x1f18, 0x1f1d}, {0x1f20, 0x1f45}, {0x1f48, 0x1f4d}, {0x1f50, 0x1f57}, {0x1f59, 0x1f59}, {0x1f5b, 0x1f5b}, {0x1f5d, 0x1f5d}, {0x1f5f, 0x1f7d},
  {0x1f80, 0x1fb4}, {0x1fb6, 0x1fbc}, {0x1fbe, 0x1fbe}, {0x1fc2, 0x1fc4}, {0x1fc6, 0x1fcc}, {0x1fd0, 0x1fd3}, {0x1fd6, 0x1fdb}, {0x1fe0, 0x1fec},
  {0x1ff2, 0x1ff4}, {0x1ff6, 0x1ffc}, {0x203c, 0x203c}, {0x2049, 0x2049}, {0x2071, 0x2071}, {0x207f, 0x207f}, {0x2090, 0x209c}, {0x2102, 0x2102},
  {0x2107, 0x2107}, {0x210a, 0x2113}, {0x2115, 0x2115}, {0x2119, 0x211d}, {0x2122, 0x2122}, {0x2124, 0x2124}, {0x2126, 0x2126}, {0x2128, 0x2128},
  {0x212a, 0x212d}, {0x212f, 0x2139}, {0x213c, 0x213f}, {0x2145, 0x2149}, {0x214e, 0x214e}, {0x2183, 0x2184}, {0x2194, 0x2199}, {0x21a9, 0x21aa},
  {0x231a, 0x231b}, {0x2328, 0x2328}, {0x23cf, 0x23cf}, {0x23e9, 0x23f3}, {0x23f8, 0x23fa}, {0x24c2, 0x24c2}, {0x25aa, 0x25ab}, {0x25b6, 0x25b6},
  {0x25c0, 0x25c0}, {0x25fb, 0x25fe}, {0x2600, 0x2604}, {0x260e, 0x260e}, {0x2611, 0x2611}, {0x2614, 0x2615}, {0x2618, 0x2618}, {0x261d, 0x261d},
  {0x2620, 0x2620}, {0x2622, 0x2623}, {0x2626, 0x2626}, {0x262a, 0x262a}, {0x262e, 0x262f}, {0x2638, 0x263a}, {0x2640, 0x2640}, {0x2642, 0x2642},
  {0x2648, 0x2653}, {0x265f, 0x2660}, {0x2663, 0x2663}, {0x2665, 0x2666}, {0x2668, 0x2668}, {0x267b, 0x267b}, {0x267e, 0x267f}, {0x2692, 0x2697},
  {0x2699, 0x2699}, {0x269b, 0x269c}, {0x26a0, 0x26a1}, {0x26a7, 0x26a7}, {0x26aa, 0x26ab}, {0x26b0, 0x26b1}, {0x26bd, 0x26be}, {0x26c4, 0x26c5},
  {0x26c8, 0x26c8}, {0x26ce, 0x26cf}, {0x26d1, 0x26d1}, {0x26d3, 0x26d4}, {0x26e9, 0x26ea}, {0x26f0, 0x26f5}, {0x26f7, 0x26fa}, {0x26fd, 0x26fd},
  {0x2702, 0x2702}, {0x2705, 0x2705}, {0x2708, 0x270d}, {0x270f, 0x270f}, {0x2712, 0x2712}, {0x2714, 0x2714}, {0x2716, 0x2716}, {0x271d, 0x271d},
  {0x2721, 0x2721}, {0x2728, 0x2728}, {0x2733, 0x2734}, {0x2744, 0x2744}, {0x2747, 0x2747}, {0x274c, 0x274c}, {0x274e, 0x274e}, {0x2753, 0x2755},
  {0x2757, 0x2757}, {0x2763, 0x2764}, {0x2795, 0x2797}, {0x27a1, 0x27a1}, {0x27b0, 0x27b0}, {0x27bf, 0x27bf}, {0x2934, 0x2935}, {0x2b05, 0x2b07},
  {0x2b1b, 0x2b1c}, {0x2b50, 0x2b50}, {0x2b55, 0x2b55}, {0x2c00, 0x2ce4}, {0x2ceb, 0x2cee}, {0x2cf2, 0x2cf3}, {0x2d00, 0x2d25}, {0x2d27, 0x2d27},
  {0x2d2d, 0x2d2d}, {0x2d30, 0x2d67}, {0x2d6f, 0x2d6f}, {0x2d80, 0x2d96}, {0x2da0, 0x2da6}, {0x2da8, 0x2dae}, {0x2db0, 0x2db6}, {0x2db8, 0x2dbe},
  {0x2dc0, 0x2dc6}, {0x2dc8, 0x2dce}, {0x2dd0, 0x2dd6}, {0x2dd8, 0x2dde}, {0x2e2f, 0x2e2f}, {0x3005, 0x3006}, {0x3030, 0x3035}, {0x303b, 0x303d},
  {0x3041, 0x3096}, {0x309d, 0x309f}, {0x30a1, 0x30fa}, {0x30fc, 0x30ff}, {0x3105, 0x312f}, {0x3131, 0x318e}, {0x31a0, 0x31bf}, {0x31f0, 0x31ff},
  {0x3297, 0x3297}, {0x3299, 0x3299}, {0x3400, 0x4dbf}, {0x4e00, 0xa48c}, {0xa4d0, 0xa4fd}, {0xa500, 0xa60c}, {0xa610, 0xa61f}, {0xa62a, 0xa62b},
  {0xa640, 0xa66e}, {0xa67f, 0xa69d}, {0xa6a0, 0xa6e5}, {0xa717, 0xa71f}, {0xa722, 0xa788}, {0xa78b, 0xa7cd}, {0xa7d0, 0xa7d1}, {0xa7d3, 0xa7d3},
  {0xa7d5, 0xa7dc}, {0xa7f2, 0xa801}, {0xa803, 0xa805}, {0xa807, 0xa80a}, {0xa80c, 0xa822}, {0xa840, 0xa873}, {0xa882, 0xa8b3}, {0xa8f2, 0xa8f7},
  {0xa8fb, 0xa8fb}, {0xa8fd, 0xa8fe}, {0xa90a, 0xa925}, {0xa930, 0xa946}, {0xa960, 0xa97c}, {0xa984, 0xa9b2}, {0xa9cf, 0xa9cf}, {0xa9e0, 0xa9e4},
  {0xa9e6, 0xa9ef}, {0xa9fa, 0xa9fe}, {0xaa00, 0xaa28}, {0xaa40, 0xaa42}, {0xaa44, 0xaa4b}, {0xaa60, 0xaa76}, {0xaa7a, 0xaa7a}, {0xaa7e, 0xaaaf},
  {0xaab1, 0xaab1}, {0xaab5, 0xaab6}, {0xaab9, 0xaabd}, {0xaac0, 0xaac0}, {0xaac2, 0xaac2}, {0xaadb, 0xaadd}, {0xaae0, 0xaaea}, {0xaaf2, 0xaaf4},
  {0xab01, 0xab06}, {0xab09, 0xab0e}, {0xab11, 0xab16}, {0xab20, 0xab26}, {0xab28, 0xab2e}, {0xab30, 0xab5a}, {0xab5c, 0xab69}, {0xab70, 0xabe2},
  {0xac00, 0xd7a3}, {0xd7b0, 0xd7c6}, {0xd7cb, 0xd7fb}, {0xf900, 0xfa6d}, {0xfa70, 0xfad9}, {0xfb00, 0xfb06}, {0xfb13, 0xfb17}, {0xfb1d, 0xfb1d},
  {0xfb1f, 0xfb28}, {0xfb2a, 0xfb36}, {0xfb38, 0xfb3c}, {0xfb3e, 0xfb3e}, {0xfb40, 0xfb41}, {0xfb43, 0xfb44}, {0xfb46, 0xfbb1}, {0xfbd3, 0xfd3d},
  {0xfd50, 0xfd8f}, {0xfd92, 0xfdc7}, {0xfdf0, 0xfdfb}, {0xfe70, 0xfe74}, {0xfe76, 0xfefc}, {0xff21, 0xff3a}, {0xff41, 0xff5a}, {0xff66, 0xffbe},
  {0xffc2, 0xffc7}, {0xffca, 0xffcf}, {0xffd2, 0xffd7}, {0xffda, 0xffdc}, {0x10000, 0x1000b}, {0x1000d, 0x10026}, {0x10028, 0x1003a}, {0x1003c, 0x1003d},
  {0x1003f, 0x1004d}, {0x10050, 0x1005d}, {0x10080, 0x100fa}, {0x10280, 0x1029c}, {0x102a0, 0x102d0}, {0x10300, 0x1031f}, {0x1032d, 0x10340}, {0x10342, 0x10349},
  {0x10350, 0x10375}, {0x10380, 0x1039d}, {0x103a0, 0x103c3}, {0x103c8, 0x103cf}, {0x10400, 0x1049d}, {0x104b0, 0x104d3}, {0x104d8, 0x104fb}, {0x10500, 0x10527},
  {0x10530, 0x10563}, {0x10570, 0x1057a}, {0x1057c, 0x1058a}, {0x1058c, 0x10592}, {0x10594, 0x10595}, {0x10597, 0x105a1}, {0x105a3, 0x105b1}, {0x105b3, 0x105b9},
  {0x105bb, 0x105bc}, {0x105c0, 0x105f3}, {0x10600, 0x10736}, {0x10740, 0x10755}, {0x10760, 0x10767}, {0x10780, 0x10785}, {0x10787, 0x107b0}, {0x107b2, 0x107ba},
  {0x10800, 0x10805}, {0x10808, 0x10808}, {0x1080a, 0x10835}, {0x10837, 0x10838}, {0x1083c, 0x1083c}, {0x1083f, 0x10855}, {0x10860, 0x10876}, {0x10880, 0x1089e},
  {0x108e0, 0x108f2}, {0x108f4, 0x108f5}, {0x10900, 0x10915}, {0x10920, 0x10939}, {0x10980, 0x109b7}, {0x109be, 0x109bf}, {0x10a00, 0x10a00}, {0x10a10, 0x10a13},
  {0x10a15, 0x10a17}, {0x10a19, 0x10a35}, {0x10a60, 0x10a7c}, {0x10a80, 0x10a9c}, {0x10ac0, 0x10ac7}, {0x10ac9, 0x10ae4}, {0x10b00, 0x10b35}, {0x10b40, 0x10b55},
  {0x10b60, 0x10b72}, {0x10b80, 0x10b91}, {0x10c00, 0x10c48}, {0x10c80, 0x10cb2}, {0x10cc0, 0x10cf2}, {0x10d00, 0x10d23}, {0x10d4a, 0x10d65}, {0x10d6f, 0x10d85},
  {0x10e80, 0x10ea9}, {0x10eb0, 0x10eb1}, {0x10ec2, 0x10ec4}, {0x10f00, 0x10f1c}, {0x10f27, 0x10f27}, {0x10f30, 0x10f45}, {0x10f70, 0x10f81}, {0x10fb0, 0x10fc4},
  {0x10fe0, 0x10ff6}, {0x11003, 0x11037}, {0x11071, 0x11072}, {0x11075, 0x11075}, {0x11083, 0x110af}, {0x110d0, 0x110e8}, {0x11103, 0x11126}, {0x11144, 0x11144},
  {0x11147, 0x11147}, {0x11150, 0x11172}, {0x11176, 0x11176}, {0x11183, 0x111b2}, {0x111c1, 0x111c4}, {0x111da, 0x111da}, {0x111dc, 0x111dc}, {0x11200, 0x11211},
  {0x11213, 0x1122b}, {0x1123f, 0x11240}, {0x11280, 0x11286}, {0x11288, 0x11288}, {0x1128a, 0x1128d}, {0x1128f, 0x1129d}, {0x1129f, 0x112a8}, {0x112b0, 0x112de},
  {0x11305, 0x1130c}, {0x1130f, 0x11310}, {0x11313, 0x11328}, {0x1132a, 0x11330}, {0x11332, 0x11333}, {0x11335, 0x11339}, {0x1133d, 0x1133d}, {0x11350, 0x11350},
  {0x1135d, 0x11361}, {0x11380, 0x11389}, {0x1138b, 0x1138b}, {0x1138e, 0x1138e}, {0x11390, 0x113b5}, {0x113b7, 0x113b7}, {0x113d1, 0x113d1}, {0x113d3, 0x113d3},
  {0x11400, 0x11434}, {0x11447, 0x1144a}, {0x1145f, 0x11461}, {0x11480, 0x114af}, {0x114c4, 0x114c5}, {0x114c7, 0x114c7}, {0x11580, 0x115ae}, {0x115d8, 0x115db},
  {0x11600, 0x1162f}, {0x11644, 0x11644}, {0x11680, 0x116aa}, {0x116b8, 0x116b8}, {0x11700, 0x1171a}, {0x11740, 0x11746}, {0x11800, 0x1182b}, {0x118a0, 0x118df},
  {0x118ff, 0x11906}, {0x11909, 0x11909}, {0x1190c, 0x11913}, {0x11915, 0x11916}, {0x11918, 0x1192f}, {0x1193f, 0x1193f}, {0x11941, 0x11941}, {0x119a0, 0x119a7},
  {0x119aa, 0x119d0}, {0x119e1, 0x119e1}, {0x119e3, 0x119e3}, {0x11a00, 0x11a00}, {0x11a0b, 0x11a32}, {0x11a3a, 0x11a3a}, {0x11a50, 0x11a50}, {0x11a5c, 0x11a89},
  {0x11a9d, 0x11a9d}, {0x11ab0, 0x11af8}, {0x11bc0, 0x11be0}, {0x11c00, 0x11c08}, {0x11c0a, 0x11c2e}, {0x11c40, 0x11c40}, {0x11c72, 0x11c8f}, {0x11d00, 0x11d06},
  {0x11d08, 0x11d09}, {0x11d0b, 0x11d30}, {0x11d46, 0x11d46}, {0x11d60, 0x11d65}, {0x11d67, 0x11d68}, {0x11d6a, 0x11d89}, {0x11d98, 0x11d98}, {0x11ee0, 0x11ef2},
  {0x11f02, 0x11f02}, {0x11f04, 0x11f10}, {0x11f12, 0x11f33}, {0x11fb0, 0x11fb0}, {0x12000, 0x12399}, {0x12480, 0x12543}, {0x12f90, 0x12ff0}, {0x13000, 0x1342f},
  {0x13441, 0x13446}, {0x13460, 0x143fa}, {0x14400, 0x14646}, {0x16100, 0x1611d}, {0x16800, 0x16a38}, {0x16a40, 0x16a5e}, {0x16a70, 0x16abe}, {0x16ad0, 0x16aed},
  {0x16b00, 0x16b2f}, {0x16b40, 0x16b43}, {0x16b63, 0x16b77}, {0x16b7d, 0x16b8f}, {0x16d40, 0x16d6c}, {0x16e40, 0x16e7f}, {0x16f00, 0x16f4a}, {0x16f50, 0x16f50},
  {0x16f93, 0x16f9f}, {0x16fe0, 0x16fe1}, {0x16fe3, 0x16fe3}, {0x17000, 0x187f7}, {0x18800, 0x18cd5}, {0x18cff, 0x18d08}, {0x1aff0, 0x1aff3}, {0x1aff5, 0x1affb},
  {0x1affd, 0x1affe}, {0x1b000, 0x1b122}, {0x1b132, 0x1b132}, {0x1b150, 0x1b152}, {0x1b155, 0x1b155}, {0x1b164, 0x1b167}, {0x1b170, 0x1b2fb}, {0x1bc00, 0x1bc6a},
  {0x1bc70, 0x1bc7c}, {0x1bc80, 0x1bc88}, {0x1bc90, 0x1bc99}, {0x1d400, 0x1d454}, {0x1d456, 0x1d49c}, {0x1d49e, 0x1d49f}, {0x1d4a2, 0x1d4a2}, {0x1d4a5, 0x1d4a6},
  {0x1d4a9, 0x1d4ac}, {0x1d4ae, 0x1d4b9}, {0x1d4bb, 0x1d4bb}, {0x1d4bd, 0x1d4c3}, {0x1d4c5, 0x1d505}, {0x1d507, 0x1d50a}, {0x1d50d, 0x1d514}, {0x1d516, 0x1d51c},
  {0x1d51e, 0x1d539}, {0x1d53b, 0x1d53e}, {0x1d540, 0x1d544}, {0x1d546, 0x1d546}, {0x1d54a, 0x1d550}, {0x1d552, 0x1d6a5}, {0x1d6a8, 0x1d6c0}, {0x1d6c2, 0x1d6da},
  {0x1d6dc, 0x1d6fa}, {0x1d6fc, 0x1d714}, {0x1d716, 0x1d734}, {0x1d736, 0x1d74e}, {0x1d750, 0x1d76e}, {0x1d770, 0x1d788}, {0x1d78a, 0x1d7a8}, {0x1d7aa, 0x1d7c2},
  {0x1d7c4, 0x1d7cb}, {0x1df00, 0x1df1e}, {0x1df25, 0x1df2a}, {0x1e030, 0x1e06d}, {0x1e100, 0x1e12c}, {0x1e137, 0x1e13d}, {0x1e14e, 0x1e14e}, {0x1e290, 0x1e2ad},
  {0x1e2c0, 0x1e2eb}, {0x1e4d0, 0x1e4eb}, {0x1e5d0, 0x1e5ed}, {0x1e5f0, 0x1e5f0}, {0x1e7e0, 0x1e7e6}, {0x1e7e8, 0x1e7eb}, {0x1e7ed, 0x1e7ee}, {0x1e7f0, 0x1e7fe},
  {0x1e800, 0x1e8c4}, {0x1e900, 0x1e943}, {0x1e94b, 0x1e94b}, {0x1ee00, 0x1ee03}, {0x1ee05, 0x1ee1f}, {0x1ee21, 0x1ee22}, {0x1ee24, 0x1ee24}, {0x1ee27, 0x1ee27},
  {0x1ee29, 0x1ee32}, {0x1ee34, 0x1ee37}, {0x1ee39, 0x1ee39}, {0x1ee3b, 0x1ee3b}, {0x1ee42, 0x1ee42}, {0x1ee47, 0x1ee47}, {0x1ee49, 0x1ee49}, {0x1ee4b, 0x1ee4b},
  {0x1ee4d, 0x1ee4f}, {0x1ee51, 0x1ee52}, {0x1ee54, 0x1ee54}, {0x1ee57, 0x1ee57}, {0x1ee59, 0x1ee59}, {0x1ee5b, 0x1ee5b}, {0x1ee5d, 0x1ee5d}, {0x1ee5f, 0x1ee5f},
  {0x1ee61, 0x1ee62}, {0x1ee64, 0x1ee64}, {0x1ee67, 0x1ee6a}, {0x1ee6c, 0x1ee72}, {0x1ee74, 0x1ee77}, {0x1ee79, 0x1ee7c}, {0x1ee7e, 0x1ee7e}, {0x1ee80, 0x1ee89},
  {0x1ee8b, 0x1ee9b}, {0x1eea1, 0x1eea3}, {0x1eea5, 0x1eea9}, {0x1eeab, 0x1eebb}, {0x1f004, 0x1f004}, {0x1f0cf, 0x1f0cf}, {0x1f170, 0x1f171}, {0x1f17e, 0x1f17f},
  {0x1f18e, 0x1f18e}, {0x1f191, 0x1f19a}, {0x1f1e6, 0x1f1ff}, {0x1f201, 0x1f202}, {0x1f21a, 0x1f21a}, {0x1f22f, 0x1f22f}, {0x1f232, 0x1f23a}, {0x1f250, 0x1f251},
  {0x1f300, 0x1f321}, {0x1f324, 0x1f393}, {0x1f396, 0x1f397}, {0x1f399, 0x1f39b}, {0x1f39e, 0x1f3f0}, {0x1f3f3, 0x1f3f5}, {0x1f3f7, 0x1f4fd}, {0x1f4ff, 0x1f53d},
  {0x1f549, 0x1f54e}, {0x1f550, 0x1f567}, {0x1f56f, 0x1f570}, {0x1f573, 0x1f57a}, {0x1f587, 0x1f587}, {0x1f58a, 0x1f58d}, {0x1f590, 0x1f590}, {0x1f595, 0x1f596},
  {0x1f5a4, 0x1f5a5}, {0x1f5a8, 0x1f5a8}, {0x1f5b1, 0x1f5b2}, {0x1f5bc, 0x1f5bc}, {0x1f5c2, 0x1f5c4}, {0x1f5d1, 0x1f5d3}, {0x1f5dc, 0x1f5de}, {0x1f5e1, 0x1f5e1},
  {0x1f5e3, 0x1f5e3}, {0x1f5e8, 0x1f5e8}, {0x1f5ef, 0x1f5ef}, {0x1f5f3, 0x1f5f3}, {0x1f5fa, 0x1f64f}, {0x1f680, 0x1f6c5}, {0x1f6cb, 0x1f6d2}, {0x1f6d5, 0x1f6d7},
  {0x1f6dc, 0x1f6e5}, {0x1f6e9, 0x1f6e9}, {0x1f6eb, 0x1f6ec}, {0x1f6f0, 0x1f6f0}, {0x1f6f3, 0x1f6fc}, {0x1f7e0, 0x1f7eb}, {0x1f7f0, 0x1f7f0}, {0x1f90c, 0x1f93a},
  {0x1f93c, 0x1f945}, {0x1f947, 0x1f9ff}, {0x1fa70, 0x1fa7c}, {0x1fa80, 0x1fa89}, {0x1fa8f, 0x1fac6}, {0x1face, 0x1fadc}, {0x1fadf, 0x1fae9}, {0x1faf0, 0x1faf8},
  {0x20000, 0x2a6df}, {0x2a700, 0x2b739}, {0x2b740, 0x2b81d}, {0x2b820, 0x2cea1}, {0x2ceb0, 0x2ebe0}, {0x2ebf0, 0x2ee5d}, {0x2f800, 0x2fa1d}, {0x30000, 0x3134a},
  {0x31350, 0x323af},
};

static const TSCharacterRange sym_identifier_character_set_2[] = {
  {'#', '#'}, {'*', '*'}, {'-', '-'}, {'/', '9'}, {'A', 'Z'}, {'_', '_'}, {'a', 'z'}, {0xa9, 0xaa},
  {0xae, 0xae}, {0xb2, 0xb3}, {0xb5, 0xb5}, {0xb9, 0xba}, {0xbc, 0xbe}, {0xc0, 0xd6}, {0xd8, 0xf6}, {0xf8, 0x2c1},
  {0x2c6, 0x2d1}, {0x2e0, 0x2e4}, {0x2ec, 0x2ec}, {0x2ee, 0x2ee}, {0x370, 0x374}, {0x376, 0x377}, {0x37a, 0x37d}, {0x37f, 0x37f},
  {0x386, 0x386}, {0x388, 0x38a}, {0x38c, 0x38c}, {0x38e, 0x3a1}, {0x3a3, 0x3f5}, {0x3f7, 0x481}, {0x48a, 0x52f}, {0x531, 0x556},
  {0x559, 0x559}, {0x560, 0x588}, {0x5d0, 0x5ea}, {0x5ef, 0x5f2}, {0x620, 0x64a}, {0x660, 0x669}, {0x66e, 0x66f}, {0x671, 0x6d3},
  {0x6d5, 0x6d5}, {0x6e5, 0x6e6}, {0x6ee, 0x6fc}, {0x6ff, 0x6ff}, {0x710, 0x710}, {0x712, 0x72f}, {0x74d, 0x7a5}, {0x7b1, 0x7b1},
  {0x7c0, 0x7ea}, {0x7f4, 0x7f5}, {0x7fa, 0x7fa}, {0x800, 0x815}, {0x81a, 0x81a}, {0x824, 0x824}, {0x828, 0x828}, {0x840, 0x858},
  {0x860, 0x86a}, {0x870, 0x887}, {0x889, 0x88e}, {0x8a0, 0x8c9}, {0x904, 0x939}, {0x93d, 0x93d}, {0x950, 0x950}, {0x958, 0x961},
  {0x966, 0x96f}, {0x971, 0x980}, {0x985, 0x98c}, {0x98f, 0x990}, {0x993, 0x9a8}, {0x9aa, 0x9b0}, {0x9b2, 0x9b2}, {0x9b6, 0x9b9},
  {0x9bd, 0x9bd}, {0x9ce, 0x9ce}, {0x9dc, 0x9dd}, {0x9df, 0x9e1}, {0x9e6, 0x9f1}, {0x9f4, 0x9f9}, {0x9fc, 0x9fc}, {0xa05, 0xa0a},
  {0xa0f, 0xa10}, {0xa13, 0xa28}, {0xa2a, 0xa30}, {0xa32, 0xa33}, {0xa35, 0xa36}, {0xa38, 0xa39}, {0xa59, 0xa5c}, {0xa5e, 0xa5e},
  {0xa66, 0xa6f}, {0xa72, 0xa74}, {0xa85, 0xa8d}, {0xa8f, 0xa91}, {0xa93, 0xaa8}, {0xaaa, 0xab0}, {0xab2, 0xab3}, {0xab5, 0xab9},
  {0xabd, 0xabd}, {0xad0, 0xad0}, {0xae0, 0xae1}, {0xae6, 0xaef}, {0xaf9, 0xaf9}, {0xb05, 0xb0c}, {0xb0f, 0xb10}, {0xb13, 0xb28},
  {0xb2a, 0xb30}, {0xb32, 0xb33}, {0xb35, 0xb39}, {0xb3d, 0xb3d}, {0xb5c, 0xb5d}, {0xb5f, 0xb61}, {0xb66, 0xb6f}, {0xb71, 0xb77},
  {0xb83, 0xb83}, {0xb85, 0xb8a}, {0xb8e, 0xb90}, {0xb92, 0xb95}, {0xb99, 0xb9a}, {0xb9c, 0xb9c}, {0xb9e, 0xb9f}, {0xba3, 0xba4},
  {0xba8, 0xbaa}, {0xbae, 0xbb9}, {0xbd0, 0xbd0}, {0xbe6, 0xbf2}, {0xc05, 0xc0c}, {0xc0e, 0xc10}, {0xc12, 0xc28}, {0xc2a, 0xc39},
  {0xc3d, 0xc3d}, {0xc58, 0xc5a}, {0xc5d, 0xc5d}, {0xc60, 0xc61}, {0xc66, 0xc6f}, {0xc78, 0xc7e}, {0xc80, 0xc80}, {0xc85, 0xc8c},
  {0xc8e, 0xc90}, {0xc92, 0xca8}, {0xcaa, 0xcb3}, {0xcb5, 0xcb9}, {0xcbd, 0xcbd}, {0xcdd, 0xcde}, {0xce0, 0xce1}, {0xce6, 0xcef},
  {0xcf1, 0xcf2}, {0xd04, 0xd0c}, {0xd0e, 0xd10}, {0xd12, 0xd3a}, {0xd3d, 0xd3d}, {0xd4e, 0xd4e}, {0xd54, 0xd56}, {0xd58, 0xd61},
  {0xd66, 0xd78}, {0xd7a, 0xd7f}, {0xd85, 0xd96}, {0xd9a, 0xdb1}, {0xdb3, 0xdbb}, {0xdbd, 0xdbd}, {0xdc0, 0xdc6}, {0xde6, 0xdef},
  {0xe01, 0xe30}, {0xe32, 0xe33}, {0xe40, 0xe46}, {0xe50, 0xe59}, {0xe81, 0xe82}, {0xe84, 0xe84}, {0xe86, 0xe8a}, {0xe8c, 0xea3},
  {0xea5, 0xea5}, {0xea7, 0xeb0}, {0xeb2, 0xeb3}, {0xebd, 0xebd}, {0xec0, 0xec4}, {0xec6, 0xec6}, {0xed0, 0xed9}, {0xedc, 0xedf},
  {0xf00, 0xf00}, {0xf20, 0xf33}, {0xf40, 0xf47}, {0xf49, 0xf6c}, {0xf88, 0xf8c}, {0x1000, 0x102a}, {0x103f, 0x1049}, {0x1050, 0x1055},
  {0x105a, 0x105d}, {0x1061, 0x1061}, {0x1065, 0x1066}, {0x106e, 0x1070}, {0x1075, 0x1081}, {0x108e, 0x108e}, {0x1090, 0x1099}, {0x10a0, 0x10c5},
  {0x10c7, 0x10c7}, {0x10cd, 0x10cd}, {0x10d0, 0x10fa}, {0x10fc, 0x1248}, {0x124a, 0x124d}, {0x1250, 0x1256}, {0x1258, 0x1258}, {0x125a, 0x125d},
  {0x1260, 0x1288}, {0x128a, 0x128d}, {0x1290, 0x12b0}, {0x12b2, 0x12b5}, {0x12b8, 0x12be}, {0x12c0, 0x12c0}, {0x12c2, 0x12c5}, {0x12c8, 0x12d6},
  {0x12d8, 0x1310}, {0x1312, 0x1315}, {0x1318, 0x135a}, {0x1369, 0x137c}, {0x1380, 0x138f}, {0x13a0, 0x13f5}, {0x13f8, 0x13fd}, {0x1401, 0x166c},
  {0x166f, 0x167f}, {0x1681, 0x169a}, {0x16a0, 0x16ea}, {0x16ee, 0x16f8}, {0x1700, 0x1711}, {0x171f, 0x1731}, {0x1740, 0x1751}, {0x1760, 0x176c},
  {0x176e, 0x1770}, {0x1780, 0x17b3}, {0x17d7, 0x17d7}, {0x17dc, 0x17dc}, {0x17e0, 0x17e9}, {0x17f0, 0x17f9}, {0x1810, 0x1819}, {0x1820, 0x1878},
  {0x1880, 0x1884}, {0x1887, 0x18a8}, {0x18aa, 0x18aa}, {0x18b0, 0x18f5}, {0x1900, 0x191e}, {0x1946, 0x196d}, {0x1970, 0x1974}, {0x1980, 0x19ab},
  {0x19b0, 0x19c9}, {0x19d0, 0x19da}, {0x1a00, 0x1a16}, {0x1a20, 0x1a54}, {0x1a80, 0x1a89}, {0x1a90, 0x1a99}, {0x1aa7, 0x1aa7}, {0x1b05, 0x1b33},
  {0x1b45, 0x1b4c}, {0x1b50, 0x1b59}, {0x1b83, 0x1ba0}, {0x1bae, 0x1be5}, {0x1c00, 0x1c23}, {0x1c40, 0x1c49}, {0x1c4d, 0x1c7d}, {0x1c80, 0x1c8a},
  {0x1c90, 0x1cba}, {0x1cbd, 0x1cbf}, {0x1ce9, 0x1cec}, {0x1cee, 0x1cf3}, {0x1cf5, 0x1cf6}, {0x1cfa, 0x1cfa}, {0x1d00, 0x1dbf}, {0x1e00, 0x1f15},
  {0x1f18, 0x1f1d}, {0x1f20, 0x1f45}, {0x1f48, 0x1f4d}, {0x1f50, 0x1f57}, {0x1f59, 0x1f59}, {0x1f5b, 0x1f5b}, {0x1f5d, 0x1f5d}, {0x1f5f, 0x1f7d},
  {0x1f80, 0x1fb4}, {0x1fb6, 0x1fbc}, {0x1fbe, 0x1fbe}, {0x1fc2, 0x1fc4}, {0x1fc6, 0x1fcc}, {0x1fd0, 0x1fd3}, {0x1fd6, 0x1fdb}, {0x1fe0, 0x1fec},
  {0x1ff2, 0x1ff4}, {0x1ff6, 0x1ffc}, {0x203c, 0x203c}, {0x2049, 0x2049}, {0x2070, 0x2071}, {0x2074, 0x2079}, {0x207f, 0x2089}, {0x2090, 0x209c},
  {0x2102, 0x2102}, {0x2107, 0x2107}, {0x210a, 0x2113}, {0x2115, 0x2115}, {0x2119, 0x211d}, {0x2122, 0x2122}, {0x2124, 0x2124}, {0x2126, 0x2126},
  {0x2128, 0x2128}, {0x212a, 0x212d}, {0x212f, 0x2139}, {0x213c, 0x213f}, {0x2145, 0x2149}, {0x214e, 0x214e}, {0x2150, 0x2189}, {0x2194, 0x2199},
  {0x21a9, 0x21aa}, {0x231a, 0x231b}, {0x2328, 0x2328}, {0x23cf, 0x23cf}, {0x23e9, 0x23f3}, {0x23f8, 0x23fa}, {0x2460, 0x249b}, {0x24c2, 0x24c2},
  {0x24ea, 0x24ff}, {0x25aa, 0x25ab}, {0x25b6, 0x25b6}, {0x25c0, 0x25c0}, {0x25fb, 0x25fe}, {0x2600, 0x2604}, {0x260e, 0x260e}, {0x2611, 0x2611},
  {0x2614, 0x2615}, {0x2618, 0x2618}, {0x261d, 0x261d}, {0x2620, 0x2620}, {0x2622, 0x2623}, {0x2626, 0x2626}, {0x262a, 0x262a}, {0x262e, 0x262f},
  {0x2638, 0x263a}, {0x2640, 0x2640}, {0x2642, 0x2642}, {0x2648, 0x2653}, {0x265f, 0x2660}, {0x2663, 0x2663}, {0x2665, 0x2666}, {0x2668, 0x2668},
  {0x267b, 0x267b}, {0x267e, 0x267f}, {0x2692, 0x2697}, {0x2699, 0x2699}, {0x269b, 0x269c}, {0x26a0, 0x26a1}, {0x26a7, 0x26a7}, {0x26aa, 0x26ab},
  {0x26b0, 0x26b1}, {0x26bd, 0x26be}, {0x26c4, 0x26c5}, {0x26c8, 0x26c8}, {0x26ce, 0x26cf}, {0x26d1, 0x26d1}, {0x26d3, 0x26d4}, {0x26e9, 0x26ea},
  {0x26f0, 0x26f5}, {0x26f7, 0x26fa}, {0x26fd, 0x26fd}, {0x2702, 0x2702}, {0x2705, 0x2705}, {0x2708, 0x270d}, {0x270f, 0x270f}, {0x2712, 0x2712},
  {0x2714, 0x2714}, {0x2716, 0x2716}, {0x271d, 0x271d}, {0x2721, 0x2721}, {0x2728, 0x2728}, {0x2733, 0x2734}, {0x2744, 0x2744}, {0x2747, 0x2747},
  {0x274c, 0x274c}, {0x274e, 0x274e}, {0x2753, 0x2755}, {0x2757, 0x2757}, {0x2763, 0x2764}, {0x2776, 0x2793}, {0x2795, 0x2797}, {0x27a1, 0x27a1},
  {0x27b0, 0x27b0}, {0x27bf, 0x27bf}, {0x2934, 0x2935}, {0x2b05, 0x2b07}, {0x2b1b, 0x2b1c}, {0x2b50, 0x2b50}, {0x2b55, 0x2b55}, {0x2c00, 0x2ce4},
  {0x2ceb, 0x2cee}, {0x2cf2, 0x2cf3}, {0x2cfd, 0x2cfd}, {0x2d00, 0x2d25}, {0x2d27, 0x2d27}, {0x2d2d, 0x2d2d}, {0x2d30, 0x2d67}, {0x2d6f, 0x2d6f},
  {0x2d80, 0x2d96}, {0x2da0, 0x2da6}, {0x2da8, 0x2dae}, {0x2db0, 0x2db6}, {0x2db8, 0x2dbe}, {0x2dc0, 0x2dc6}, {0x2dc8, 0x2dce}, {0x2dd0, 0x2dd6},
  {0x2dd8, 0x2dde}, {0x2e2f, 0x2e2f}, {0x3005, 0x3007}, {0x3021, 0x3029}, {0x3030, 0x3035}, {0x3038, 0x303d}, {0x3041, 0x3096}, {0x309d, 0x309f},
  {0x30a1, 0x30fa}, {0x30fc, 0x30ff}, {0x3105, 0x312f}, {0x3131, 0x318e}, {0x3192, 0x3195}, {0x31a0, 0x31bf}, {0x31f0, 0x31ff}, {0x3220, 0x3229},
  {0x3248, 0x324f}, {0x3251, 0x325f}, {0x3280, 0x3289}, {0x3297, 0x3297}, {0x3299, 0x3299}, {0x32b1, 0x32bf}, {0x3400, 0x4dbf}, {0x4e00, 0xa48c},
  {0xa4d0, 0xa4fd}, {0xa500, 0xa60c}, {0xa610, 0xa62b}, {0xa640, 0xa66e}, {0xa67f, 0xa69d}, {0xa6a0, 0xa6ef}, {0xa717, 0xa71f}, {0xa722, 0xa788},
  {0xa78b, 0xa7cd}, {0xa7d0, 0xa7d1}, {0xa7d3, 0xa7d3}, {0xa7d5, 0xa7dc}, {0xa7f2, 0xa801}, {0xa803, 0xa805}, {0xa807, 0xa80a}, {0xa80c, 0xa822},
  {0xa830, 0xa835}, {0xa840, 0xa873}, {0xa882, 0xa8b3}, {0xa8d0, 0xa8d9}, {0xa8f2, 0xa8f7}, {0xa8fb, 0xa8fb}, {0xa8fd, 0xa8fe}, {0xa900, 0xa925},
  {0xa930, 0xa946}, {0xa960, 0xa97c}, {0xa984, 0xa9b2}, {0xa9cf, 0xa9d9}, {0xa9e0, 0xa9e4}, {0xa9e6, 0xa9fe}, {0xaa00, 0xaa28}, {0xaa40, 0xaa42},
  {0xaa44, 0xaa4b}, {0xaa50, 0xaa59}, {0xaa60, 0xaa76}, {0xaa7a, 0xaa7a}, {0xaa7e, 0xaaaf}, {0xaab1, 0xaab1}, {0xaab5, 0xaab6}, {0xaab9, 0xaabd},
  {0xaac0, 0xaac0}, {0xaac2, 0xaac2}, {0xaadb, 0xaadd}, {0xaae0, 0xaaea}, {0xaaf2, 0xaaf4}, {0xab01, 0xab06}, {0xab09, 0xab0e}, {0xab11, 0xab16},
  {0xab20, 0xab26}, {0xab28, 0xab2e}, {0xab30, 0xab5a}, {0xab5c, 0xab69}, {0xab70, 0xabe2}, {0xabf0, 0xabf9}, {0xac00, 0xd7a3}, {0xd7b0, 0xd7c6},
  {0xd7cb, 0xd7fb}, {0xf900, 0xfa6d}, {0xfa70, 0xfad9}, {0xfb00, 0xfb06}, {0xfb13, 0xfb17}, {0xfb1d, 0xfb1d}, {0xfb1f, 0xfb28}, {0xfb2a, 0xfb36},
  {0xfb38, 0xfb3c}, {0xfb3e, 0xfb3e}, {0xfb40, 0xfb41}, {0xfb43, 0xfb44}, {0xfb46, 0xfbb1}, {0xfbd3, 0xfd3d}, {0xfd50, 0xfd8f}, {0xfd92, 0xfdc7},
  {0xfdf0, 0xfdfb}, {0xfe70, 0xfe74}, {0xfe76, 0xfefc}, {0xff10, 0xff19}, {0xff21, 0xff3a}, {0xff41, 0xff5a}, {0xff66, 0xffbe}, {0xffc2, 0xffc7},
  {0xffca, 0xffcf}, {0xffd2, 0xffd7}, {0xffda, 0xffdc}, {0x10000, 0x1000b}, {0x1000d, 0x10026}, {0x10028, 0x1003a}, {0x1003c, 0x1003d}, {0x1003f, 0x1004d},
  {0x10050, 0x1005d}, {0x10080, 0x100fa}, {0x10107, 0x10133}, {0x10140, 0x10178}, {0x1018a, 0x1018b}, {0x10280, 0x1029c}, {0x102a0, 0x102d0}, {0x102e1, 0x102fb},
  {0x10300, 0x10323}, {0x1032d, 0x1034a}, {0x10350, 0x10375}, {0x10380, 0x1039d}, {0x103a0, 0x103c3}, {0x103c8, 0x103cf}, {0x103d1, 0x103d5}, {0x10400, 0x1049d},
  {0x104a0, 0x104a9}, {0x104b0, 0x104d3}, {0x104d8, 0x104fb}, {0x10500, 0x10527}, {0x10530, 0x10563}, {0x10570, 0x1057a}, {0x1057c, 0x1058a}, {0x1058c, 0x10592},
  {0x10594, 0x10595}, {0x10597, 0x105a1}, {0x105a3, 0x105b1}, {0x105b3, 0x105b9}, {0x105bb, 0x105bc}, {0x105c0, 0x105f3}, {0x10600, 0x10736}, {0x10740, 0x10755},
  {0x10760, 0x10767}, {0x10780, 0x10785}, {0x10787, 0x107b0}, {0x107b2, 0x107ba}, {0x10800, 0x10805}, {0x10808, 0x10808}, {0x1080a, 0x10835}, {0x10837, 0x10838},
  {0x1083c, 0x1083c}, {0x1083f, 0x10855}, {0x10858, 0x10876}, {0x10879, 0x1089e}, {0x108a7, 0x108af}, {0x108e0, 0x108f2}, {0x108f4, 0x108f5}, {0x108fb, 0x1091b},
  {0x10920, 0x10939}, {0x10980, 0x109b7}, {0x109bc, 0x109cf}, {0x109d2, 0x10a00}, {0x10a10, 0x10a13}, {0x10a15, 0x10a17}, {0x10a19, 0x10a35}, {0x10a40, 0x10a48},
  {0x10a60, 0x10a7e}, {0x10a80, 0x10a9f}, {0x10ac0, 0x10ac7}, {0x10ac9, 0x10ae4}, {0x10aeb, 0x10aef}, {0x10b00, 0x10b35}, {0x10b40, 0x10b55}, {0x10b58, 0x10b72},
  {0x10b78, 0x10b91}, {0x10ba9, 0x10baf}, {0x10c00, 0x10c48}, {0x10c80, 0x10cb2}, {0x10cc0, 0x10cf2}, {0x10cfa, 0x10d23}, {0x10d30, 0x10d39}, {0x10d40, 0x10d65},
  {0x10d6f, 0x10d85}, {0x10e60, 0x10e7e}, {0x10e80, 0x10ea9}, {0x10eb0, 0x10eb1}, {0x10ec2, 0x10ec4}, {0x10f00, 0x10f27}, {0x10f30, 0x10f45}, {0x10f51, 0x10f54},
  {0x10f70, 0x10f81}, {0x10fb0, 0x10fcb}, {0x10fe0, 0x10ff6}, {0x11003, 0x11037}, {0x11052, 0x1106f}, {0x11071, 0x11072}, {0x11075, 0x11075}, {0x11083, 0x110af},
  {0x110d0, 0x110e8}, {0x110f0, 0x110f9}, {0x11103, 0x11126}, {0x11136, 0x1113f}, {0x11144, 0x11144}, {0x11147, 0x11147}, {0x11150, 0x11172}, {0x11176, 0x11176},
  {0x11183, 0x111b2}, {0x111c1, 0x111c4}, {0x111d0, 0x111da}, {0x111dc, 0x111dc}, {0x111e1, 0x111f4}, {0x11200, 0x11211}, {0x11213, 0x1122b}, {0x1123f, 0x11240},
  {0x11280, 0x11286}, {0x11288, 0x11288}, {0x1128a, 0x1128d}, {0x1128f, 0x1129d}, {0x1129f, 0x112a8}, {0x112b0, 0x112de}, {0x112f0, 0x112f9}, {0x11305, 0x1130c},
  {0x1130f, 0x11310}, {0x11313, 0x11328}, {0x1132a, 0x11330}, {0x11332, 0x11333}, {0x11335, 0x11339}, {0x1133d, 0x1133d}, {0x11350, 0x11350}, {0x1135d, 0x11361},
  {0x11380, 0x11389}, {0x1138b, 0x1138b}, {0x1138e, 0x1138e}, {0x11390, 0x113b5}, {0x113b7, 0x113b7}, {0x113d1, 0x113d1}, {0x113d3, 0x113d3}, {0x11400, 0x11434},
  {0x11447, 0x1144a}, {0x11450, 0x11459}, {0x1145f, 0x11461}, {0x11480, 0x114af}, {0x114c4, 0x114c5}, {0x114c7, 0x114c7}, {0x114d0, 0x114d9}, {0x11580, 0x115ae},
  {0x115d8, 0x115db}, {0x11600, 0x1162f}, {0x11644, 0x11644}, {0x11650, 0x11659}, {0x11680, 0x116aa}, {0x116b8, 0x116b8}, {0x116c0, 0x116c9}, {0x116d0, 0x116e3},
  {0x11700, 0x1171a}, {0x11730, 0x1173b}, {0x11740, 0x11746}, {0x11800, 0x1182b}, {0x118a0, 0x118f2}, {0x118ff, 0x11906}, {0x11909, 0x11909}, {0x1190c, 0x11913},
  {0x11915, 0x11916}, {0x11918, 0x1192f}, {0x1193f, 0x1193f}, {0x11941, 0x11941}, {0x11950, 0x11959}, {0x119a0, 0x119a7}, {0x119aa, 0x119d0}, {0x119e1, 0x119e1},
  {0x119e3, 0x119e3}, {0x11a00, 0x11a00}, {0x11a0b, 0x11a32}, {0x11a3a, 0x11a3a}, {0x11a50, 0x11a50}, {0x11a5c, 0x11a89}, {0x11a9d, 0x11a9d}, {0x11ab0, 0x11af8},
  {0x11bc0, 0x11be0}, {0x11bf0, 0x11bf9}, {0x11c00, 0x11c08}, {0x11c0a, 0x11c2e}, {0x11c40, 0x11c40}, {0x11c50, 0x11c6c}, {0x11c72, 0x11c8f}, {0x11d00, 0x11d06},
  {0x11d08, 0x11d09}, {0x11d0b, 0x11d30}, {0x11d46, 0x11d46}, {0x11d50, 0x11d59}, {0x11d60, 0x11d65}, {0x11d67, 0x11d68}, {0x11d6a, 0x11d89}, {0x11d98, 0x11d98},
  {0x11da0, 0x11da9}, {0x11ee0, 0x11ef2}, {0x11f02, 0x11f02}, {0x11f04, 0x11f10}, {0x11f12, 0x11f33}, {0x11f50, 0x11f59}, {0x11fb0, 0x11fb0}, {0x11fc0, 0x11fd4},
  {0x12000, 0x12399}, {0x12400, 0x1246e}, {0x12480, 0x12543}, {0x12f90, 0x12ff0}, {0x13000, 0x1342f}, {0x13441, 0x13446}, {0x13460, 0x143fa}, {0x14400, 0x14646},
  {0x16100, 0x1611d}, {0x16130, 0x16139}, {0x16800, 0x16a38}, {0x16a40, 0x16a5e}, {0x16a60, 0x16a69}, {0x16a70, 0x16abe}, {0x16ac0, 0x16ac9}, {0x16ad0, 0x16aed},
  {0x16b00, 0x16b2f}, {0x16b40, 0x16b43}, {0x16b50, 0x16b59}, {0x16b5b, 0x16b61}, {0x16b63, 0x16b77}, {0x16b7d, 0x16b8f}, {0x16d40, 0x16d6c}, {0x16d70, 0x16d79},
  {0x16e40, 0x16e96}, {0x16f00, 0x16f4a}, {0x16f50, 0x16f50}, {0x16f93, 0x16f9f}, {0x16fe0, 0x16fe1}, {0x16fe3, 0x16fe3}, {0x17000, 0x187f7}, {0x18800, 0x18cd5},
  {0x18cff, 0x18d08}, {0x1aff0, 0x1aff3}, {0x1aff5, 0x1affb}, {0x1affd, 0x1affe}, {0x1b000, 0x1b122}, {0x1b132, 0x1b132}, {0x1b150, 0x1b152}, {0x1b155, 0x1b155},
  {0x1b164, 0x1b167}, {0x1b170, 0x1b2fb}, {0x1bc00, 0x1bc6a}, {0x1bc70, 0x1bc7c}, {0x1bc80, 0x1bc88}, {0x1bc90, 0x1bc99}, {0x1ccf0, 0x1ccf9}, {0x1d2c0, 0x1d2d3},
  {0x1d2e0, 0x1d2f3}, {0x1d360, 0x1d378}, {0x1d400, 0x1d454}, {0x1d456, 0x1d49c}, {0x1d49e, 0x1d49f}, {0x1d4a2, 0x1d4a2}, {0x1d4a5, 0x1d4a6}, {0x1d4a9, 0x1d4ac},
  {0x1d4ae, 0x1d4b9}, {0x1d4bb, 0x1d4bb}, {0x1d4bd, 0x1d4c3}, {0x1d4c5, 0x1d505}, {0x1d507, 0x1d50a}, {0x1d50d, 0x1d514}, {0x1d516, 0x1d51c}, {0x1d51e, 0x1d539},
  {0x1d53b, 0x1d53e}, {0x1d540, 0x1d544}, {0x1d546, 0x1d546}, {0x1d54a, 0x1d550}, {0x1d552, 0x1d6a5}, {0x1d6a8, 0x1d6c0}, {0x1d6c2, 0x1d6da}, {0x1d6dc, 0x1d6fa},
  {0x1d6fc, 0x1d714}, {0x1d716, 0x1d734}, {0x1d736, 0x1d74e}, {0x1d750, 0x1d76e}, {0x1d770, 0x1d788}, {0x1d78a, 0x1d7a8}, {0x1d7aa, 0x1d7c2}, {0x1d7c4, 0x1d7cb},
  {0x1d7ce, 0x1d7ff}, {0x1df00, 0x1df1e}, {0x1df25, 0x1df2a}, {0x1e030, 0x1e06d}, {0x1e100, 0x1e12c}, {0x1e137, 0x1e13d}, {0x1e140, 0x1e149}, {0x1e14e, 0x1e14e},
  {0x1e290, 0x1e2ad}, {0x1e2c0, 0x1e2eb}, {0x1e2f0, 0x1e2f9}, {0x1e4d0, 0x1e4eb}, {0x1e4f0, 0x1e4f9}, {0x1e5d0, 0x1e5ed}, {0x1e5f0, 0x1e5fa}, {0x1e7e0, 0x1e7e6},
  {0x1e7e8, 0x1e7eb}, {0x1e7ed, 0x1e7ee}, {0x1e7f0, 0x1e7fe}, {0x1e800, 0x1e8c4}, {0x1e8c7, 0x1e8cf}, {0x1e900, 0x1e943}, {0x1e94b, 0x1e94b}, {0x1e950, 0x1e959},
  {0x1ec71, 0x1ecab}, {0x1ecad, 0x1ecaf}, {0x1ecb1, 0x1ecb4}, {0x1ed01, 0x1ed2d}, {0x1ed2f, 0x1ed3d}, {0x1ee00, 0x1ee03}, {0x1ee05, 0x1ee1f}, {0x1ee21, 0x1ee22},
  {0x1ee24, 0x1ee24}, {0x1ee27, 0x1ee27}, {0x1ee29, 0x1ee32}, {0x1ee34, 0x1ee37}, {0x1ee39, 0x1ee39}, {0x1ee3b, 0x1ee3b}, {0x1ee42, 0x1ee42}, {0x1ee47, 0x1ee47},
  {0x1ee49, 0x1ee49}, {0x1ee4b, 0x1ee4b}, {0x1ee4d, 0x1ee4f}, {0x1ee51, 0x1ee52}, {0x1ee54, 0x1ee54}, {0x1ee57, 0x1ee57}, {0x1ee59, 0x1ee59}, {0x1ee5b, 0x1ee5b},
  {0x1ee5d, 0x1ee5d}, {0x1ee5f, 0x1ee5f}, {0x1ee61, 0x1ee62}, {0x1ee64, 0x1ee64}, {0x1ee67, 0x1ee6a}, {0x1ee6c, 0x1ee72}, {0x1ee74, 0x1ee77}, {0x1ee79, 0x1ee7c},
  {0x1ee7e, 0x1ee7e}, {0x1ee80, 0x1ee89}, {0x1ee8b, 0x1ee9b}, {0x1eea1, 0x1eea3}, {0x1eea5, 0x1eea9}, {0x1eeab, 0x1eebb}, {0x1f004, 0x1f004}, {0x1f0cf, 0x1f0cf},
  {0x1f100, 0x1f10c}, {0x1f170, 0x1f171}, {0x1f17e, 0x1f17f}, {0x1f18e, 0x1f18e}, {0x1f191, 0x1f19a}, {0x1f1e6, 0x1f1ff}, {0x1f201, 0x1f202}, {0x1f21a, 0x1f21a},
  {0x1f22f, 0x1f22f}, {0x1f232, 0x1f23a}, {0x1f250, 0x1f251}, {0x1f300, 0x1f321}, {0x1f324, 0x1f393}, {0x1f396, 0x1f397}, {0x1f399, 0x1f39b}, {0x1f39e, 0x1f3f0},
  {0x1f3f3, 0x1f3f5}, {0x1f3f7, 0x1f4fd}, {0x1f4ff, 0x1f53d}, {0x1f549, 0x1f54e}, {0x1f550, 0x1f567}, {0x1f56f, 0x1f570}, {0x1f573, 0x1f57a}, {0x1f587, 0x1f587},
  {0x1f58a, 0x1f58d}, {0x1f590, 0x1f590}, {0x1f595, 0x1f596}, {0x1f5a4, 0x1f5a5}, {0x1f5a8, 0x1f5a8}, {0x1f5b1, 0x1f5b2}, {0x1f5bc, 0x1f5bc}, {0x1f5c2, 0x1f5c4},
  {0x1f5d1, 0x1f5d3}, {0x1f5dc, 0x1f5de}, {0x1f5e1, 0x1f5e1}, {0x1f5e3, 0x1f5e3}, {0x1f5e8, 0x1f5e8}, {0x1f5ef, 0x1f5ef}, {0x1f5f3, 0x1f5f3}, {0x1f5fa, 0x1f64f},
  {0x1f680, 0x1f6c5}, {0x1f6cb, 0x1f6d2}, {0x1f6d5, 0x1f6d7}, {0x1f6dc, 0x1f6e5}, {0x1f6e9, 0x1f6e9}, {0x1f6eb, 0x1f6ec}, {0x1f6f0, 0x1f6f0}, {0x1f6f3, 0x1f6fc},
  {0x1f7e0, 0x1f7eb}, {0x1f7f0, 0x1f7f0}, {0x1f90c, 0x1f93a}, {0x1f93c, 0x1f945}, {0x1f947, 0x1f9ff}, {0x1fa70, 0x1fa7c}, {0x1fa80, 0x1fa89}, {0x1fa8f, 0x1fac6},
  {0x1face, 0x1fadc}, {0x1fadf, 0x1fae9}, {0x1faf0, 0x1faf8}, {0x1fbf0, 0x1fbf9}, {0x20000, 0x2a6df}, {0x2a700, 0x2b739}, {0x2b740, 0x2b81d}, {0x2b820, 0x2cea1},
  {0x2ceb0, 0x2ebe0}, {0x2ebf0, 0x2ee5d}, {0x2f800, 0x2fa1d}, {0x30000, 0x3134a}, {0x31350, 0x323af},
};

static bool ts_lex(TSLexer *lexer, TSStateId state) {
  START_LEXER();
  eof = lexer->eof(lexer);
  switch (state) {
    case 0:
      if (eof) ADVANCE(10);
      if (lookahead == '\t') SKIP(0);
      if (lookahead == ' ') ADVANCE(13);
      if (lookahead == '!') ADVANCE(26);
      if (lookahead == '$') ADVANCE(9);
      if (lookahead == '*') ADVANCE(21);
      if (lookahead == '+') ADVANCE(5);
      if (lookahead == '-') ADVANCE(18);
      if (lookahead == '.') ADVANCE(8);
      if (lookahead == '/') ADVANCE(19);
      if (lookahead == '<') ADVANCE(6);
      if (lookahead == '=') ADVANCE(7);
      if (lookahead == '?') ADVANCE(25);
      if (lookahead == '@') ADVANCE(23);
      if (lookahead == 'I') ADVANCE(41);
      if (lookahead == 'N') ADVANCE(28);
      if (lookahead == '\\') ADVANCE(24);
      if (lookahead == '^') ADVANCE(22);
      if (lookahead == 'f') ADVANCE(29);
      if (lookahead == 'n') ADVANCE(50);
      if (lookahead == 't') ADVANCE(45);
      if (('0' <= lookahead && lookahead <= '9')) ADVANCE(56);
      if (set_contains(sym_identifier_character_set_1, 825, lookahead)) ADVANCE(53);
      END_STATE();
    case 1:
      if (lookahead == '\t') SKIP(1);
      if (lookahead == ' ') ADVANCE(13);
      if (lookahead == '!') ADVANCE(26);
      if (lookahead == '*') ADVANCE(20);
      if (lookahead == '?') ADVANCE(25);
      END_STATE();
    case 2:
      if (lookahead == '\t') ADVANCE(11);
      if (lookahead != 0 &&
          lookahead != '\t' &&
          lookahead != '\n' &&
          lookahead != '\r') ADVANCE(12);
      END_STATE();
    case 3:
      if (lookahead == '\t') SKIP(3);
      if (lookahead == ' ') ADVANCE(13);
      if (lookahead == '$') ADVANCE(9);
      if (lookahead == '-') ADVANCE(4);
      if (set_contains(sym_identifier_character_set_1, 825, lookahead)) ADVANCE(53);
      END_STATE();
    case 4:
      if (lookahead == '-') ADVANCE(4);
      if (set_contains(sym_identifier_character_set_1, 825, lookahead)) ADVANCE(53);
      END_STATE();
    case 5:
      if (lookahead == '.') ADVANCE(8);
      if (('0' <= lookahead && lookahead <= '9')) ADVANCE(56);
      END_STATE();
    case 6:
      if (lookahead == '=') ADVANCE(16);
      END_STATE();
    case 7:
      if (lookahead == '>') ADVANCE(17);
      END_STATE();
    case 8:
      if (('0' <= lookahead && lookahead <= '9')) ADVANCE(54);
      END_STATE();
    case 9:
      if (('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(14);
      END_STATE();
    case 10:
      ACCEPT_TOKEN(ts_builtin_sym_end);
      END_STATE();
    case 11:
      ACCEPT_TOKEN(aux_sym_comment_content_token1);
      if (lookahead == '\t') ADVANCE(11);
      if (lookahead != 0 &&
          lookahead != '\t' &&
          lookahead != '\n' &&
          lookahead != '\r') ADVANCE(12);
      END_STATE();
    case 12:
      ACCEPT_TOKEN(aux_sym_comment_content_token1);
      if (lookahead != 0 &&
          lookahead != '\n' &&
          lookahead != '\r') ADVANCE(12);
      END_STATE();
    case 13:
      ACCEPT_TOKEN(anon_sym_SPACE);
      END_STATE();
    case 14:
      ACCEPT_TOKEN(sym_component_name);
      if (('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(14);
      END_STATE();
    case 15:
      ACCEPT_TOKEN(anon_sym_LT_EQ_GT);
      END_STATE();
    case 16:
      ACCEPT_TOKEN(anon_sym_LT_EQ);
      if (lookahead == '>') ADVANCE(15);
      END_STATE();
    case 17:
      ACCEPT_TOKEN(anon_sym_EQ_GT);
      END_STATE();
    case 18:
      ACCEPT_TOKEN(sym_comment_marker);
      if (lookahead == '-') ADVANCE(4);
      if (lookahead == '.') ADVANCE(8);
      if (lookahead == 'I') ADVANCE(44);
      if (('0' <= lookahead && lookahead <= '9')) ADVANCE(56);
      if (set_contains(sym_identifier_character_set_1, 825, lookahead)) ADVANCE(53);
      END_STATE();
    case 19:
      ACCEPT_TOKEN(anon_sym_SLASH);
      END_STATE();
    case 20:
      ACCEPT_TOKEN(anon_sym_STAR);
      END_STATE();
    case 21:
      ACCEPT_TOKEN(anon_sym_STAR);
      if (set_contains(sym_identifier_character_set_2, 917, lookahead)) ADVANCE(53);
      END_STATE();
    case 22:
      ACCEPT_TOKEN(anon_sym_CARET);
      END_STATE();
    case 23:
      ACCEPT_TOKEN(anon_sym_AT);
      END_STATE();
    case 24:
      ACCEPT_TOKEN(aux_sym_localized_text_token1);
      if (lookahead != 0 &&
          lookahead != '\n' &&
          lookahead != '\r') ADVANCE(24);
      END_STATE();
    case 25:
      ACCEPT_TOKEN(anon_sym_QMARK);
      END_STATE();
    case 26:
      ACCEPT_TOKEN(anon_sym_BANG);
      END_STATE();
    case 27:
      ACCEPT_TOKEN(sym_identifier);
      if (lookahead == 'N') ADVANCE(57);
      if (set_contains(sym_identifier_character_set_2, 917, lookahead)) ADVANCE(53);
      END_STATE();
    case 28:
      ACCEPT_TOKEN(sym_identifier);
      if (lookahead == 'a') ADVANCE(27);
      if (set_contains(sym_identifier_character_set_2, 917, lookahead)) ADVANCE(53);
      END_STATE();
    case 29:
      ACCEPT_TOKEN(sym_identifier);
      if (lookahead == 'a') ADVANCE(38);
      if (set_contains(sym_identifier_character_set_2, 917, lookahead)) ADVANCE(53);
      END_STATE();
    case 30:
      ACCEPT_TOKEN(sym_identifier);
      if (lookahead == 'e') ADVANCE(60);
      if (set_contains(sym_identifier_character_set_2, 917, lookahead)) ADVANCE(53);
      END_STATE();
    case 31:
      ACCEPT_TOKEN(sym_identifier);
      if (lookahead == 'e') ADVANCE(61);
      if (set_contains(sym_identifier_character_set_2, 917, lookahead)) ADVANCE(53);
      END_STATE();
    case 32:
      ACCEPT_TOKEN(sym_identifier);
      if (lookahead == 'f') ADVANCE(35);
      if (set_contains(sym_identifier_character_set_2, 917, lookahead)) ADVANCE(53);
      END_STATE();
    case 33:
      ACCEPT_TOKEN(sym_identifier);
      if (lookahead == 'f') ADVANCE(37);
      if (set_contains(sym_identifier_character_set_2, 917, lookahead)) ADVANCE(53);
      END_STATE();
    case 34:
      ACCEPT_TOKEN(sym_identifier);
      if (lookahead == 'i') ADVANCE(47);
      if (set_contains(sym_identifier_character_set_2, 917, lookahead)) ADVANCE(53);
      END_STATE();
    case 35:
      ACCEPT_TOKEN(sym_identifier);
      if (lookahead == 'i') ADVANCE(42);
      if (set_contains(sym_identifier_character_set_2, 917, lookahead)) ADVANCE(53);
      END_STATE();
    case 36:
      ACCEPT_TOKEN(sym_identifier);
      if (lookahead == 'i') ADVANCE(48);
      if (set_contains(sym_identifier_character_set_2, 917, lookahead)) ADVANCE(53);
      END_STATE();
    case 37:
      ACCEPT_TOKEN(sym_identifier);
      if (lookahead == 'i') ADVANCE(43);
      if (set_contains(sym_identifier_character_set_2, 917, lookahead)) ADVANCE(53);
      END_STATE();
    case 38:
      ACCEPT_TOKEN(sym_identifier);
      if (lookahead == 'l') ADVANCE(46);
      if (set_contains(sym_identifier_character_set_2, 917, lookahead)) ADVANCE(53);
      END_STATE();
    case 39:
      ACCEPT_TOKEN(sym_identifier);
      if (lookahead == 'l') ADVANCE(62);
      if (set_contains(sym_identifier_character_set_2, 917, lookahead)) ADVANCE(53);
      END_STATE();
    case 40:
      ACCEPT_TOKEN(sym_identifier);
      if (lookahead == 'l') ADVANCE(39);
      if (set_contains(sym_identifier_character_set_2, 917, lookahead)) ADVANCE(53);
      END_STATE();
    case 41:
      ACCEPT_TOKEN(sym_identifier);
      if (lookahead == 'n') ADVANCE(32);
      if (set_contains(sym_identifier_character_set_2, 917, lookahead)) ADVANCE(53);
      END_STATE();
    case 42:
      ACCEPT_TOKEN(sym_identifier);
      if (lookahead == 'n') ADVANCE(34);
      if (set_contains(sym_identifier_character_set_2, 917, lookahead)) ADVANCE(53);
      END_STATE();
    case 43:
      ACCEPT_TOKEN(sym_identifier);
      if (lookahead == 'n') ADVANCE(36);
      if (set_contains(sym_identifier_character_set_2, 917, lookahead)) ADVANCE(53);
      END_STATE();
    case 44:
      ACCEPT_TOKEN(sym_identifier);
      if (lookahead == 'n') ADVANCE(33);
      if (set_contains(sym_identifier_character_set_2, 917, lookahead)) ADVANCE(53);
      END_STATE();
    case 45:
      ACCEPT_TOKEN(sym_identifier);
      if (lookahead == 'r') ADVANCE(49);
      if (set_contains(sym_identifier_character_set_2, 917, lookahead)) ADVANCE(53);
      END_STATE();
    case 46:
      ACCEPT_TOKEN(sym_identifier);
      if (lookahead == 's') ADVANCE(31);
      if (set_contains(sym_identifier_character_set_2, 917, lookahead)) ADVANCE(53);
      END_STATE();
    case 47:
      ACCEPT_TOKEN(sym_identifier);
      if (lookahead == 't') ADVANCE(51);
      if (set_contains(sym_identifier_character_set_2, 917, lookahead)) ADVANCE(53);
      END_STATE();
    case 48:
      ACCEPT_TOKEN(sym_identifier);
      if (lookahead == 't') ADVANCE(52);
      if (set_contains(sym_identifier_character_set_2, 917, lookahead)) ADVANCE(53);
      END_STATE();
    case 49:
      ACCEPT_TOKEN(sym_identifier);
      if (lookahead == 'u') ADVANCE(30);
      if (set_contains(sym_identifier_character_set_2, 917, lookahead)) ADVANCE(53);
      END_STATE();
    case 50:
      ACCEPT_TOKEN(sym_identifier);
      if (lookahead == 'u') ADVANCE(40);
      if (set_contains(sym_identifier_character_set_2, 917, lookahead)) ADVANCE(53);
      END_STATE();
    case 51:
      ACCEPT_TOKEN(sym_identifier);
      if (lookahead == 'y') ADVANCE(58);
      if (set_contains(sym_identifier_character_set_2, 917, lookahead)) ADVANCE(53);
      END_STATE();
    case 52:
      ACCEPT_TOKEN(sym_identifier);
      if (lookahead == 'y') ADVANCE(59);
      if (set_contains(sym_identifier_character_set_2, 917, lookahead)) ADVANCE(53);
      END_STATE();
    case 53:
      ACCEPT_TOKEN(sym_identifier);
      if (set_contains(sym_identifier_character_set_2, 917, lookahead)) ADVANCE(53);
      END_STATE();
    case 54:
      ACCEPT_TOKEN(aux_sym_number_token1);
      if (('0' <= lookahead && lookahead <= '9')) ADVANCE(54);
      END_STATE();
    case 55:
      ACCEPT_TOKEN(aux_sym_number_token2);
      END_STATE();
    case 56:
      ACCEPT_TOKEN(aux_sym_number_token2);
      if (lookahead == '.') ADVANCE(8);
      if (lookahead == 'n') ADVANCE(55);
      if (('0' <= lookahead && lookahead <= '9')) ADVANCE(56);
      END_STATE();
    case 57:
      ACCEPT_TOKEN(anon_sym_NaN);
      if (set_contains(sym_identifier_character_set_2, 917, lookahead)) ADVANCE(53);
      END_STATE();
    case 58:
      ACCEPT_TOKEN(anon_sym_Infinity);
      if (set_contains(sym_identifier_character_set_2, 917, lookahead)) ADVANCE(53);
      END_STATE();
    case 59:
      ACCEPT_TOKEN(anon_sym_DASHInfinity);
      if (set_contains(sym_identifier_character_set_2, 917, lookahead)) ADVANCE(53);
      END_STATE();
    case 60:
      ACCEPT_TOKEN(anon_sym_true);
      if (set_contains(sym_identifier_character_set_2, 917, lookahead)) ADVANCE(53);
      END_STATE();
    case 61:
      ACCEPT_TOKEN(anon_sym_false);
      if (set_contains(sym_identifier_character_set_2, 917, lookahead)) ADVANCE(53);
      END_STATE();
    case 62:
      ACCEPT_TOKEN(sym_null);
      if (set_contains(sym_identifier_character_set_2, 917, lookahead)) ADVANCE(53);
      END_STATE();
    default:
      return false;
  }
}

static const TSLexerMode ts_lex_modes[STATE_COUNT] = {
  [0] = {.lex_state = 0, .external_lex_state = 1},
  [1] = {.lex_state = 0},
  [2] = {.lex_state = 0},
  [3] = {.lex_state = 0},
  [4] = {.lex_state = 0, .external_lex_state = 2},
  [5] = {.lex_state = 0, .external_lex_state = 2},
  [6] = {.lex_state = 0, .external_lex_state = 2},
  [7] = {.lex_state = 0},
  [8] = {.lex_state = 0},
  [9] = {.lex_state = 0},
  [10] = {.lex_state = 0, .external_lex_state = 3},
  [11] = {.lex_state = 0, .external_lex_state = 3},
  [12] = {.lex_state = 0, .external_lex_state = 3},
  [13] = {.lex_state = 0, .external_lex_state = 4},
  [14] = {.lex_state = 0, .external_lex_state = 4},
  [15] = {.lex_state = 0, .external_lex_state = 4},
  [16] = {.lex_state = 0},
  [17] = {.lex_state = 0},
  [18] = {.lex_state = 0},
  [19] = {.lex_state = 0},
  [20] = {.lex_state = 0, .external_lex_state = 2},
  [21] = {.lex_state = 0},
  [22] = {.lex_state = 0},
  [23] = {.lex_state = 0, .external_lex_state = 2},
  [24] = {.lex_state = 0, .external_lex_state = 2},
  [25] = {.lex_state = 0, .external_lex_state = 2},
  [26] = {.lex_state = 0, .external_lex_state = 2},
  [27] = {.lex_state = 0, .external_lex_state = 2},
  [28] = {.lex_state = 1, .external_lex_state = 5},
  [29] = {.lex_state = 2, .external_lex_state = 1},
  [30] = {.lex_state = 2, .external_lex_state = 1},
  [31] = {.lex_state = 2, .external_lex_state = 6},
  [32] = {.lex_state = 2, .external_lex_state = 6},
  [33] = {.lex_state = 2, .external_lex_state = 6},
  [34] = {.lex_state = 2, .external_lex_state = 6},
  [35] = {.lex_state = 2, .external_lex_state = 5},
  [36] = {.lex_state = 2, .external_lex_state = 5},
  [37] = {.lex_state = 3, .external_lex_state = 5},
  [38] = {.lex_state = 2, .external_lex_state = 5},
  [39] = {.lex_state = 2, .external_lex_state = 6},
  [40] = {.lex_state = 3, .external_lex_state = 5},
  [41] = {.lex_state = 2, .external_lex_state = 6},
  [42] = {.lex_state = 2, .external_lex_state = 5},
  [43] = {.lex_state = 0, .external_lex_state = 5},
  [44] = {.lex_state = 3, .external_lex_state = 5},
  [45] = {.lex_state = 2, .external_lex_state = 5},
  [46] = {.lex_state = 2, .external_lex_state = 6},
  [47] = {.lex_state = 0, .external_lex_state = 5},
  [48] = {.lex_state = 0, .external_lex_state = 5},
  [49] = {.lex_state = 0, .external_lex_state = 5},
  [50] = {.lex_state = 0, .external_lex_state = 5},
  [51] = {.lex_state = 0},
  [52] = {.lex_state = 0, .external_lex_state = 5},
  [53] = {.lex_state = 0, .external_lex_state = 5},
  [54] = {.lex_state = 0, .external_lex_state = 5},
  [55] = {.lex_state = 0, .external_lex_state = 5},
  [56] = {.lex_state = 0, .external_lex_state = 5},
  [57] = {.lex_state = 0, .external_lex_state = 5},
  [58] = {.lex_state = 0, .external_lex_state = 5},
  [59] = {.lex_state = 0, .external_lex_state = 5},
  [60] = {.lex_state = 0, .external_lex_state = 5},
  [61] = {.lex_state = 0, .external_lex_state = 5},
  [62] = {.lex_state = 0, .external_lex_state = 5},
  [63] = {.lex_state = 0, .external_lex_state = 5},
  [64] = {.lex_state = 0, .external_lex_state = 5},
  [65] = {.lex_state = 0},
  [66] = {.lex_state = 0},
};

static const uint16_t ts_parse_table[LARGE_STATE_COUNT][SYMBOL_COUNT] = {
  [STATE(0)] = {
    [ts_builtin_sym_end] = ACTIONS(1),
    [anon_sym_SPACE] = ACTIONS(1),
    [sym_component_name] = ACTIONS(1),
    [anon_sym_LT_EQ_GT] = ACTIONS(1),
    [anon_sym_LT_EQ] = ACTIONS(1),
    [anon_sym_EQ_GT] = ACTIONS(1),
    [sym_comment_marker] = ACTIONS(1),
    [anon_sym_SLASH] = ACTIONS(1),
    [anon_sym_STAR] = ACTIONS(1),
    [anon_sym_CARET] = ACTIONS(1),
    [anon_sym_AT] = ACTIONS(1),
    [aux_sym_localized_text_token1] = ACTIONS(1),
    [anon_sym_QMARK] = ACTIONS(1),
    [anon_sym_BANG] = ACTIONS(1),
    [sym_identifier] = ACTIONS(1),
    [aux_sym_number_token1] = ACTIONS(1),
    [aux_sym_number_token2] = ACTIONS(1),
    [anon_sym_NaN] = ACTIONS(1),
    [anon_sym_Infinity] = ACTIONS(1),
    [anon_sym_DASHInfinity] = ACTIONS(1),
    [anon_sym_true] = ACTIONS(1),
    [anon_sym_false] = ACTIONS(1),
    [sym_null] = ACTIONS(1),
    [sym__indent] = ACTIONS(1),
    [sym__dedent] = ACTIONS(1),
    [sym__newline] = ACTIONS(1),
  },
  [STATE(1)] = {
    [sym_source_file] = STATE(66),
    [sym_line] = STATE(3),
    [sym_commented_line] = STATE(16),
    [sym_normal_line] = STATE(16),
    [sym_token_chain] = STATE(64),
    [sym__token] = STATE(54),
    [sym_binding_operator] = STATE(54),
    [sym_typed_list] = STATE(54),
    [sym_typed_dict] = STATE(54),
    [sym_list_marker] = STATE(54),
    [sym_dict_marker] = STATE(54),
    [sym_localized_string] = STATE(54),
    [sym_string_literal] = STATE(54),
    [sym_identifier_with_modifier] = STATE(54),
    [sym_number] = STATE(54),
    [sym_boolean] = STATE(54),
    [aux_sym_source_file_repeat1] = STATE(3),
    [ts_builtin_sym_end] = ACTIONS(3),
    [sym_component_name] = ACTIONS(5),
    [anon_sym_LT_EQ_GT] = ACTIONS(7),
    [anon_sym_LT_EQ] = ACTIONS(9),
    [anon_sym_EQ_GT] = ACTIONS(7),
    [sym_comment_marker] = ACTIONS(11),
    [anon_sym_SLASH] = ACTIONS(13),
    [anon_sym_STAR] = ACTIONS(15),
    [anon_sym_CARET] = ACTIONS(17),
    [anon_sym_AT] = ACTIONS(19),
    [aux_sym_localized_text_token1] = ACTIONS(21),
    [sym_identifier] = ACTIONS(23),
    [aux_sym_number_token1] = ACTIONS(25),
    [aux_sym_number_token2] = ACTIONS(27),
    [anon_sym_NaN] = ACTIONS(27),
    [anon_sym_Infinity] = ACTIONS(27),
    [anon_sym_DASHInfinity] = ACTIONS(27),
    [anon_sym_true] = ACTIONS(29),
    [anon_sym_false] = ACTIONS(29),
    [sym_null] = ACTIONS(31),
  },
  [STATE(2)] = {
    [sym_line] = STATE(2),
    [sym_commented_line] = STATE(16),
    [sym_normal_line] = STATE(16),
    [sym_token_chain] = STATE(64),
    [sym__token] = STATE(54),
    [sym_binding_operator] = STATE(54),
    [sym_typed_list] = STATE(54),
    [sym_typed_dict] = STATE(54),
    [sym_list_marker] = STATE(54),
    [sym_dict_marker] = STATE(54),
    [sym_localized_string] = STATE(54),
    [sym_string_literal] = STATE(54),
    [sym_identifier_with_modifier] = STATE(54),
    [sym_number] = STATE(54),
    [sym_boolean] = STATE(54),
    [aux_sym_source_file_repeat1] = STATE(2),
    [ts_builtin_sym_end] = ACTIONS(33),
    [sym_component_name] = ACTIONS(35),
    [anon_sym_LT_EQ_GT] = ACTIONS(38),
    [anon_sym_LT_EQ] = ACTIONS(41),
    [anon_sym_EQ_GT] = ACTIONS(38),
    [sym_comment_marker] = ACTIONS(44),
    [anon_sym_SLASH] = ACTIONS(47),
    [anon_sym_STAR] = ACTIONS(50),
    [anon_sym_CARET] = ACTIONS(53),
    [anon_sym_AT] = ACTIONS(56),
    [aux_sym_localized_text_token1] = ACTIONS(59),
    [sym_identifier] = ACTIONS(62),
    [aux_sym_number_token1] = ACTIONS(65),
    [aux_sym_number_token2] = ACTIONS(68),
    [anon_sym_NaN] = ACTIONS(68),
    [anon_sym_Infinity] = ACTIONS(68),
    [anon_sym_DASHInfinity] = ACTIONS(68),
    [anon_sym_true] = ACTIONS(71),
    [anon_sym_false] = ACTIONS(71),
    [sym_null] = ACTIONS(74),
  },
  [STATE(3)] = {
    [sym_line] = STATE(2),
    [sym_commented_line] = STATE(16),
    [sym_normal_line] = STATE(16),
    [sym_token_chain] = STATE(64),
    [sym__token] = STATE(54),
    [sym_binding_operator] = STATE(54),
    [sym_typed_list] = STATE(54),
    [sym_typed_dict] = STATE(54),
    [sym_list_marker] = STATE(54),
    [sym_dict_marker] = STATE(54),
    [sym_localized_string] = STATE(54),
    [sym_string_literal] = STATE(54),
    [sym_identifier_with_modifier] = STATE(54),
    [sym_number] = STATE(54),
    [sym_boolean] = STATE(54),
    [aux_sym_source_file_repeat1] = STATE(2),
    [ts_builtin_sym_end] = ACTIONS(77),
    [sym_component_name] = ACTIONS(5),
    [anon_sym_LT_EQ_GT] = ACTIONS(7),
    [anon_sym_LT_EQ] = ACTIONS(9),
    [anon_sym_EQ_GT] = ACTIONS(7),
    [sym_comment_marker] = ACTIONS(11),
    [anon_sym_SLASH] = ACTIONS(13),
    [anon_sym_STAR] = ACTIONS(15),
    [anon_sym_CARET] = ACTIONS(17),
    [anon_sym_AT] = ACTIONS(19),
    [aux_sym_localized_text_token1] = ACTIONS(21),
    [sym_identifier] = ACTIONS(23),
    [aux_sym_number_token1] = ACTIONS(25),
    [aux_sym_number_token2] = ACTIONS(27),
    [anon_sym_NaN] = ACTIONS(27),
    [anon_sym_Infinity] = ACTIONS(27),
    [anon_sym_DASHInfinity] = ACTIONS(27),
    [anon_sym_true] = ACTIONS(29),
    [anon_sym_false] = ACTIONS(29),
    [sym_null] = ACTIONS(31),
  },
  [STATE(4)] = {
    [sym_line] = STATE(5),
    [sym_commented_line] = STATE(20),
    [sym_normal_line] = STATE(20),
    [sym_token_chain] = STATE(61),
    [sym__token] = STATE(54),
    [sym_binding_operator] = STATE(54),
    [sym_typed_list] = STATE(54),
    [sym_typed_dict] = STATE(54),
    [sym_list_marker] = STATE(54),
    [sym_dict_marker] = STATE(54),
    [sym_localized_string] = STATE(54),
    [sym_string_literal] = STATE(54),
    [sym_identifier_with_modifier] = STATE(54),
    [sym_number] = STATE(54),
    [sym_boolean] = STATE(54),
    [aux_sym_source_file_repeat1] = STATE(5),
    [sym_component_name] = ACTIONS(5),
    [anon_sym_LT_EQ_GT] = ACTIONS(7),
    [anon_sym_LT_EQ] = ACTIONS(9),
    [anon_sym_EQ_GT] = ACTIONS(7),
    [sym_comment_marker] = ACTIONS(79),
    [anon_sym_SLASH] = ACTIONS(13),
    [anon_sym_STAR] = ACTIONS(15),
    [anon_sym_CARET] = ACTIONS(17),
    [anon_sym_AT] = ACTIONS(19),
    [aux_sym_localized_text_token1] = ACTIONS(21),
    [sym_identifier] = ACTIONS(23),
    [aux_sym_number_token1] = ACTIONS(25),
    [aux_sym_number_token2] = ACTIONS(27),
    [anon_sym_NaN] = ACTIONS(27),
    [anon_sym_Infinity] = ACTIONS(27),
    [anon_sym_DASHInfinity] = ACTIONS(27),
    [anon_sym_true] = ACTIONS(29),
    [anon_sym_false] = ACTIONS(29),
    [sym_null] = ACTIONS(31),
    [sym__dedent] = ACTIONS(81),
  },
  [STATE(5)] = {
    [sym_line] = STATE(5),
    [sym_commented_line] = STATE(20),
    [sym_normal_line] = STATE(20),
    [sym_token_chain] = STATE(61),
    [sym__token] = STATE(54),
    [sym_binding_operator] = STATE(54),
    [sym_typed_list] = STATE(54),
    [sym_typed_dict] = STATE(54),
    [sym_list_marker] = STATE(54),
    [sym_dict_marker] = STATE(54),
    [sym_localized_string] = STATE(54),
    [sym_string_literal] = STATE(54),
    [sym_identifier_with_modifier] = STATE(54),
    [sym_number] = STATE(54),
    [sym_boolean] = STATE(54),
    [aux_sym_source_file_repeat1] = STATE(5),
    [sym_component_name] = ACTIONS(35),
    [anon_sym_LT_EQ_GT] = ACTIONS(38),
    [anon_sym_LT_EQ] = ACTIONS(41),
    [anon_sym_EQ_GT] = ACTIONS(38),
    [sym_comment_marker] = ACTIONS(83),
    [anon_sym_SLASH] = ACTIONS(47),
    [anon_sym_STAR] = ACTIONS(50),
    [anon_sym_CARET] = ACTIONS(53),
    [anon_sym_AT] = ACTIONS(56),
    [aux_sym_localized_text_token1] = ACTIONS(59),
    [sym_identifier] = ACTIONS(62),
    [aux_sym_number_token1] = ACTIONS(65),
    [aux_sym_number_token2] = ACTIONS(68),
    [anon_sym_NaN] = ACTIONS(68),
    [anon_sym_Infinity] = ACTIONS(68),
    [anon_sym_DASHInfinity] = ACTIONS(68),
    [anon_sym_true] = ACTIONS(71),
    [anon_sym_false] = ACTIONS(71),
    [sym_null] = ACTIONS(74),
    [sym__dedent] = ACTIONS(33),
  },
  [STATE(6)] = {
    [sym_line] = STATE(5),
    [sym_commented_line] = STATE(20),
    [sym_normal_line] = STATE(20),
    [sym_token_chain] = STATE(61),
    [sym__token] = STATE(54),
    [sym_binding_operator] = STATE(54),
    [sym_typed_list] = STATE(54),
    [sym_typed_dict] = STATE(54),
    [sym_list_marker] = STATE(54),
    [sym_dict_marker] = STATE(54),
    [sym_localized_string] = STATE(54),
    [sym_string_literal] = STATE(54),
    [sym_identifier_with_modifier] = STATE(54),
    [sym_number] = STATE(54),
    [sym_boolean] = STATE(54),
    [aux_sym_source_file_repeat1] = STATE(5),
    [sym_component_name] = ACTIONS(5),
    [anon_sym_LT_EQ_GT] = ACTIONS(7),
    [anon_sym_LT_EQ] = ACTIONS(9),
    [anon_sym_EQ_GT] = ACTIONS(7),
    [sym_comment_marker] = ACTIONS(79),
    [anon_sym_SLASH] = ACTIONS(13),
    [anon_sym_STAR] = ACTIONS(15),
    [anon_sym_CARET] = ACTIONS(17),
    [anon_sym_AT] = ACTIONS(19),
    [aux_sym_localized_text_token1] = ACTIONS(21),
    [sym_identifier] = ACTIONS(23),
    [aux_sym_number_token1] = ACTIONS(25),
    [aux_sym_number_token2] = ACTIONS(27),
    [anon_sym_NaN] = ACTIONS(27),
    [anon_sym_Infinity] = ACTIONS(27),
    [anon_sym_DASHInfinity] = ACTIONS(27),
    [anon_sym_true] = ACTIONS(29),
    [anon_sym_false] = ACTIONS(29),
    [sym_null] = ACTIONS(31),
    [sym__dedent] = ACTIONS(86),
  },
  [STATE(7)] = {
    [sym_line] = STATE(4),
    [sym_commented_line] = STATE(20),
    [sym_normal_line] = STATE(20),
    [sym_token_chain] = STATE(61),
    [sym__token] = STATE(54),
    [sym_binding_operator] = STATE(54),
    [sym_typed_list] = STATE(54),
    [sym_typed_dict] = STATE(54),
    [sym_list_marker] = STATE(54),
    [sym_dict_marker] = STATE(54),
    [sym_localized_string] = STATE(54),
    [sym_string_literal] = STATE(54),
    [sym_identifier_with_modifier] = STATE(54),
    [sym_number] = STATE(54),
    [sym_boolean] = STATE(54),
    [aux_sym_source_file_repeat1] = STATE(4),
    [sym_component_name] = ACTIONS(5),
    [anon_sym_LT_EQ_GT] = ACTIONS(7),
    [anon_sym_LT_EQ] = ACTIONS(9),
    [anon_sym_EQ_GT] = ACTIONS(7),
    [sym_comment_marker] = ACTIONS(79),
    [anon_sym_SLASH] = ACTIONS(13),
    [anon_sym_STAR] = ACTIONS(15),
    [anon_sym_CARET] = ACTIONS(17),
    [anon_sym_AT] = ACTIONS(19),
    [aux_sym_localized_text_token1] = ACTIONS(21),
    [sym_identifier] = ACTIONS(23),
    [aux_sym_number_token1] = ACTIONS(25),
    [aux_sym_number_token2] = ACTIONS(27),
    [anon_sym_NaN] = ACTIONS(27),
    [anon_sym_Infinity] = ACTIONS(27),
    [anon_sym_DASHInfinity] = ACTIONS(27),
    [anon_sym_true] = ACTIONS(29),
    [anon_sym_false] = ACTIONS(29),
    [sym_null] = ACTIONS(31),
  },
  [STATE(8)] = {
    [sym_line] = STATE(6),
    [sym_commented_line] = STATE(20),
    [sym_normal_line] = STATE(20),
    [sym_token_chain] = STATE(61),
    [sym__token] = STATE(54),
    [sym_binding_operator] = STATE(54),
    [sym_typed_list] = STATE(54),
    [sym_typed_dict] = STATE(54),
    [sym_list_marker] = STATE(54),
    [sym_dict_marker] = STATE(54),
    [sym_localized_string] = STATE(54),
    [sym_string_literal] = STATE(54),
    [sym_identifier_with_modifier] = STATE(54),
    [sym_number] = STATE(54),
    [sym_boolean] = STATE(54),
    [aux_sym_source_file_repeat1] = STATE(6),
    [sym_component_name] = ACTIONS(5),
    [anon_sym_LT_EQ_GT] = ACTIONS(7),
    [anon_sym_LT_EQ] = ACTIONS(9),
    [anon_sym_EQ_GT] = ACTIONS(7),
    [sym_comment_marker] = ACTIONS(79),
    [anon_sym_SLASH] = ACTIONS(13),
    [anon_sym_STAR] = ACTIONS(15),
    [anon_sym_CARET] = ACTIONS(17),
    [anon_sym_AT] = ACTIONS(19),
    [aux_sym_localized_text_token1] = ACTIONS(21),
    [sym_identifier] = ACTIONS(23),
    [aux_sym_number_token1] = ACTIONS(25),
    [aux_sym_number_token2] = ACTIONS(27),
    [anon_sym_NaN] = ACTIONS(27),
    [anon_sym_Infinity] = ACTIONS(27),
    [anon_sym_DASHInfinity] = ACTIONS(27),
    [anon_sym_true] = ACTIONS(29),
    [anon_sym_false] = ACTIONS(29),
    [sym_null] = ACTIONS(31),
  },
  [STATE(9)] = {
    [sym_token_chain] = STATE(59),
    [sym__token] = STATE(54),
    [sym_binding_operator] = STATE(54),
    [sym_typed_list] = STATE(54),
    [sym_typed_dict] = STATE(54),
    [sym_list_marker] = STATE(54),
    [sym_dict_marker] = STATE(54),
    [sym_localized_string] = STATE(54),
    [sym_string_literal] = STATE(54),
    [sym_identifier_with_modifier] = STATE(54),
    [sym_number] = STATE(54),
    [sym_boolean] = STATE(54),
    [sym_component_name] = ACTIONS(5),
    [anon_sym_LT_EQ_GT] = ACTIONS(7),
    [anon_sym_LT_EQ] = ACTIONS(9),
    [anon_sym_EQ_GT] = ACTIONS(7),
    [anon_sym_SLASH] = ACTIONS(13),
    [anon_sym_STAR] = ACTIONS(15),
    [anon_sym_CARET] = ACTIONS(17),
    [anon_sym_AT] = ACTIONS(19),
    [aux_sym_localized_text_token1] = ACTIONS(21),
    [sym_identifier] = ACTIONS(23),
    [aux_sym_number_token1] = ACTIONS(25),
    [aux_sym_number_token2] = ACTIONS(27),
    [anon_sym_NaN] = ACTIONS(27),
    [anon_sym_Infinity] = ACTIONS(27),
    [anon_sym_DASHInfinity] = ACTIONS(27),
    [anon_sym_true] = ACTIONS(29),
    [anon_sym_false] = ACTIONS(29),
    [sym_null] = ACTIONS(31),
  },
};

static const uint16_t ts_small_parse_table[] = {
  [0] = 4,
    ACTIONS(92), 1,
      sym__indent,
    STATE(19), 1,
      sym_commented_children,
    ACTIONS(88), 9,
      ts_builtin_sym_end,
      sym_component_name,
      anon_sym_LT_EQ_GT,
      anon_sym_EQ_GT,
      anon_sym_SLASH,
      anon_sym_CARET,
      anon_sym_AT,
      aux_sym_localized_text_token1,
      aux_sym_number_token1,
    ACTIONS(90), 11,
      anon_sym_LT_EQ,
      sym_comment_marker,
      anon_sym_STAR,
      sym_identifier,
      aux_sym_number_token2,
      anon_sym_NaN,
      anon_sym_Infinity,
      anon_sym_DASHInfinity,
      anon_sym_true,
      anon_sym_false,
      sym_null,
  [31] = 4,
    ACTIONS(98), 1,
      sym__indent,
    STATE(17), 1,
      sym_children,
    ACTIONS(94), 9,
      ts_builtin_sym_end,
      sym_component_name,
      anon_sym_LT_EQ_GT,
      anon_sym_EQ_GT,
      anon_sym_SLASH,
      anon_sym_CARET,
      anon_sym_AT,
      aux_sym_localized_text_token1,
      aux_sym_number_token1,
    ACTIONS(96), 11,
      anon_sym_LT_EQ,
      sym_comment_marker,
      anon_sym_STAR,
      sym_identifier,
      aux_sym_number_token2,
      anon_sym_NaN,
      anon_sym_Infinity,
      anon_sym_DASHInfinity,
      anon_sym_true,
      anon_sym_false,
      sym_null,
  [62] = 4,
    ACTIONS(92), 1,
      sym__indent,
    STATE(18), 1,
      sym_commented_children,
    ACTIONS(100), 9,
      ts_builtin_sym_end,
      sym_component_name,
      anon_sym_LT_EQ_GT,
      anon_sym_EQ_GT,
      anon_sym_SLASH,
      anon_sym_CARET,
      anon_sym_AT,
      aux_sym_localized_text_token1,
      aux_sym_number_token1,
    ACTIONS(102), 11,
      anon_sym_LT_EQ,
      sym_comment_marker,
      anon_sym_STAR,
      sym_identifier,
      aux_sym_number_token2,
      anon_sym_NaN,
      anon_sym_Infinity,
      anon_sym_DASHInfinity,
      anon_sym_true,
      anon_sym_false,
      sym_null,
  [93] = 4,
    ACTIONS(104), 1,
      sym__indent,
    STATE(24), 1,
      sym_children,
    ACTIONS(94), 9,
      sym__dedent,
      sym_component_name,
      anon_sym_LT_EQ_GT,
      anon_sym_EQ_GT,
      anon_sym_SLASH,
      anon_sym_CARET,
      anon_sym_AT,
      aux_sym_localized_text_token1,
      aux_sym_number_token1,
    ACTIONS(96), 11,
      anon_sym_LT_EQ,
      sym_comment_marker,
      anon_sym_STAR,
      sym_identifier,
      aux_sym_number_token2,
      anon_sym_NaN,
      anon_sym_Infinity,
      anon_sym_DASHInfinity,
      anon_sym_true,
      anon_sym_false,
      sym_null,
  [124] = 4,
    ACTIONS(106), 1,
      sym__indent,
    STATE(23), 1,
      sym_commented_children,
    ACTIONS(100), 9,
      sym__dedent,
      sym_component_name,
      anon_sym_LT_EQ_GT,
      anon_sym_EQ_GT,
      anon_sym_SLASH,
      anon_sym_CARET,
      anon_sym_AT,
      aux_sym_localized_text_token1,
      aux_sym_number_token1,
    ACTIONS(102), 11,
      anon_sym_LT_EQ,
      sym_comment_marker,
      anon_sym_STAR,
      sym_identifier,
      aux_sym_number_token2,
      anon_sym_NaN,
      anon_sym_Infinity,
      anon_sym_DASHInfinity,
      anon_sym_true,
      anon_sym_false,
      sym_null,
  [155] = 4,
    ACTIONS(106), 1,
      sym__indent,
    STATE(25), 1,
      sym_commented_children,
    ACTIONS(88), 9,
      sym__dedent,
      sym_component_name,
      anon_sym_LT_EQ_GT,
      anon_sym_EQ_GT,
      anon_sym_SLASH,
      anon_sym_CARET,
      anon_sym_AT,
      aux_sym_localized_text_token1,
      aux_sym_number_token1,
    ACTIONS(90), 11,
      anon_sym_LT_EQ,
      sym_comment_marker,
      anon_sym_STAR,
      sym_identifier,
      aux_sym_number_token2,
      anon_sym_NaN,
      anon_sym_Infinity,
      anon_sym_DASHInfinity,
      anon_sym_true,
      anon_sym_false,
      sym_null,
  [186] = 2,
    ACTIONS(108), 9,
      ts_builtin_sym_end,
      sym_component_name,
      anon_sym_LT_EQ_GT,
      anon_sym_EQ_GT,
      anon_sym_SLASH,
      anon_sym_CARET,
      anon_sym_AT,
      aux_sym_localized_text_token1,
      aux_sym_number_token1,
    ACTIONS(110), 11,
      anon_sym_LT_EQ,
      sym_comment_marker,
      anon_sym_STAR,
      sym_identifier,
      aux_sym_number_token2,
      anon_sym_NaN,
      anon_sym_Infinity,
      anon_sym_DASHInfinity,
      anon_sym_true,
      anon_sym_false,
      sym_null,
  [211] = 2,
    ACTIONS(112), 9,
      ts_builtin_sym_end,
      sym_component_name,
      anon_sym_LT_EQ_GT,
      anon_sym_EQ_GT,
      anon_sym_SLASH,
      anon_sym_CARET,
      anon_sym_AT,
      aux_sym_localized_text_token1,
      aux_sym_number_token1,
    ACTIONS(114), 11,
      anon_sym_LT_EQ,
      sym_comment_marker,
      anon_sym_STAR,
      sym_identifier,
      aux_sym_number_token2,
      anon_sym_NaN,
      anon_sym_Infinity,
      anon_sym_DASHInfinity,
      anon_sym_true,
      anon_sym_false,
      sym_null,
  [236] = 2,
    ACTIONS(88), 9,
      ts_builtin_sym_end,
      sym_component_name,
      anon_sym_LT_EQ_GT,
      anon_sym_EQ_GT,
      anon_sym_SLASH,
      anon_sym_CARET,
      anon_sym_AT,
      aux_sym_localized_text_token1,
      aux_sym_number_token1,
    ACTIONS(90), 11,
      anon_sym_LT_EQ,
      sym_comment_marker,
      anon_sym_STAR,
      sym_identifier,
      aux_sym_number_token2,
      anon_sym_NaN,
      anon_sym_Infinity,
      anon_sym_DASHInfinity,
      anon_sym_true,
      anon_sym_false,
      sym_null,
  [261] = 2,
    ACTIONS(116), 9,
      ts_builtin_sym_end,
      sym_component_name,
      anon_sym_LT_EQ_GT,
      anon_sym_EQ_GT,
      anon_sym_SLASH,
      anon_sym_CARET,
      anon_sym_AT,
      aux_sym_localized_text_token1,
      aux_sym_number_token1,
    ACTIONS(118), 11,
      anon_sym_LT_EQ,
      sym_comment_marker,
      anon_sym_STAR,
      sym_identifier,
      aux_sym_number_token2,
      anon_sym_NaN,
      anon_sym_Infinity,
      anon_sym_DASHInfinity,
      anon_sym_true,
      anon_sym_false,
      sym_null,
  [286] = 2,
    ACTIONS(108), 9,
      sym__dedent,
      sym_component_name,
      anon_sym_LT_EQ_GT,
      anon_sym_EQ_GT,
      anon_sym_SLASH,
      anon_sym_CARET,
      anon_sym_AT,
      aux_sym_localized_text_token1,
      aux_sym_number_token1,
    ACTIONS(110), 11,
      anon_sym_LT_EQ,
      sym_comment_marker,
      anon_sym_STAR,
      sym_identifier,
      aux_sym_number_token2,
      anon_sym_NaN,
      anon_sym_Infinity,
      anon_sym_DASHInfinity,
      anon_sym_true,
      anon_sym_false,
      sym_null,
  [311] = 2,
    ACTIONS(120), 9,
      ts_builtin_sym_end,
      sym_component_name,
      anon_sym_LT_EQ_GT,
      anon_sym_EQ_GT,
      anon_sym_SLASH,
      anon_sym_CARET,
      anon_sym_AT,
      aux_sym_localized_text_token1,
      aux_sym_number_token1,
    ACTIONS(122), 11,
      anon_sym_LT_EQ,
      sym_comment_marker,
      anon_sym_STAR,
      sym_identifier,
      aux_sym_number_token2,
      anon_sym_NaN,
      anon_sym_Infinity,
      anon_sym_DASHInfinity,
      anon_sym_true,
      anon_sym_false,
      sym_null,
  [336] = 2,
    ACTIONS(124), 9,
      ts_builtin_sym_end,
      sym_component_name,
      anon_sym_LT_EQ_GT,
      anon_sym_EQ_GT,
      anon_sym_SLASH,
      anon_sym_CARET,
      anon_sym_AT,
      aux_sym_localized_text_token1,
      aux_sym_number_token1,
    ACTIONS(126), 11,
      anon_sym_LT_EQ,
      sym_comment_marker,
      anon_sym_STAR,
      sym_identifier,
      aux_sym_number_token2,
      anon_sym_NaN,
      anon_sym_Infinity,
      anon_sym_DASHInfinity,
      anon_sym_true,
      anon_sym_false,
      sym_null,
  [361] = 2,
    ACTIONS(88), 9,
      sym__dedent,
      sym_component_name,
      anon_sym_LT_EQ_GT,
      anon_sym_EQ_GT,
      anon_sym_SLASH,
      anon_sym_CARET,
      anon_sym_AT,
      aux_sym_localized_text_token1,
      aux_sym_number_token1,
    ACTIONS(90), 11,
      anon_sym_LT_EQ,
      sym_comment_marker,
      anon_sym_STAR,
      sym_identifier,
      aux_sym_number_token2,
      anon_sym_NaN,
      anon_sym_Infinity,
      anon_sym_DASHInfinity,
      anon_sym_true,
      anon_sym_false,
      sym_null,
  [386] = 2,
    ACTIONS(112), 9,
      sym__dedent,
      sym_component_name,
      anon_sym_LT_EQ_GT,
      anon_sym_EQ_GT,
      anon_sym_SLASH,
      anon_sym_CARET,
      anon_sym_AT,
      aux_sym_localized_text_token1,
      aux_sym_number_token1,
    ACTIONS(114), 11,
      anon_sym_LT_EQ,
      sym_comment_marker,
      anon_sym_STAR,
      sym_identifier,
      aux_sym_number_token2,
      anon_sym_NaN,
      anon_sym_Infinity,
      anon_sym_DASHInfinity,
      anon_sym_true,
      anon_sym_false,
      sym_null,
  [411] = 2,
    ACTIONS(116), 9,
      sym__dedent,
      sym_component_name,
      anon_sym_LT_EQ_GT,
      anon_sym_EQ_GT,
      anon_sym_SLASH,
      anon_sym_CARET,
      anon_sym_AT,
      aux_sym_localized_text_token1,
      aux_sym_number_token1,
    ACTIONS(118), 11,
      anon_sym_LT_EQ,
      sym_comment_marker,
      anon_sym_STAR,
      sym_identifier,
      aux_sym_number_token2,
      anon_sym_NaN,
      anon_sym_Infinity,
      anon_sym_DASHInfinity,
      anon_sym_true,
      anon_sym_false,
      sym_null,
  [436] = 2,
    ACTIONS(124), 9,
      sym__dedent,
      sym_component_name,
      anon_sym_LT_EQ_GT,
      anon_sym_EQ_GT,
      anon_sym_SLASH,
      anon_sym_CARET,
      anon_sym_AT,
      aux_sym_localized_text_token1,
      aux_sym_number_token1,
    ACTIONS(126), 11,
      anon_sym_LT_EQ,
      sym_comment_marker,
      anon_sym_STAR,
      sym_identifier,
      aux_sym_number_token2,
      anon_sym_NaN,
      anon_sym_Infinity,
      anon_sym_DASHInfinity,
      anon_sym_true,
      anon_sym_false,
      sym_null,
  [461] = 2,
    ACTIONS(120), 9,
      sym__dedent,
      sym_component_name,
      anon_sym_LT_EQ_GT,
      anon_sym_EQ_GT,
      anon_sym_SLASH,
      anon_sym_CARET,
      anon_sym_AT,
      aux_sym_localized_text_token1,
      aux_sym_number_token1,
    ACTIONS(122), 11,
      anon_sym_LT_EQ,
      sym_comment_marker,
      anon_sym_STAR,
      sym_identifier,
      aux_sym_number_token2,
      anon_sym_NaN,
      anon_sym_Infinity,
      anon_sym_DASHInfinity,
      anon_sym_true,
      anon_sym_false,
      sym_null,
  [486] = 3,
    STATE(40), 1,
      sym_property_modifier,
    ACTIONS(128), 2,
      sym__newline,
      anon_sym_SPACE,
    ACTIONS(130), 3,
      anon_sym_STAR,
      anon_sym_QMARK,
      anon_sym_BANG,
  [499] = 3,
    ACTIONS(134), 1,
      sym__indent,
    STATE(41), 1,
      sym_commented_children,
    ACTIONS(132), 3,
      sym__dedent,
      sym__newline,
      aux_sym_comment_content_token1,
  [511] = 3,
    ACTIONS(134), 1,
      sym__indent,
    STATE(39), 1,
      sym_commented_children,
    ACTIONS(136), 3,
      sym__dedent,
      sym__newline,
      aux_sym_comment_content_token1,
  [523] = 4,
    ACTIONS(138), 1,
      aux_sym_comment_content_token1,
    ACTIONS(140), 1,
      sym__dedent,
    ACTIONS(142), 1,
      sym__newline,
    STATE(32), 2,
      sym_commented_child_line,
      aux_sym_commented_children_repeat1,
  [537] = 4,
    ACTIONS(144), 1,
      aux_sym_comment_content_token1,
    ACTIONS(147), 1,
      sym__dedent,
    ACTIONS(149), 1,
      sym__newline,
    STATE(32), 2,
      sym_commented_child_line,
      aux_sym_commented_children_repeat1,
  [551] = 4,
    ACTIONS(138), 1,
      aux_sym_comment_content_token1,
    ACTIONS(142), 1,
      sym__newline,
    ACTIONS(152), 1,
      sym__dedent,
    STATE(32), 2,
      sym_commented_child_line,
      aux_sym_commented_children_repeat1,
  [565] = 4,
    ACTIONS(138), 1,
      aux_sym_comment_content_token1,
    ACTIONS(142), 1,
      sym__newline,
    ACTIONS(154), 1,
      sym__dedent,
    STATE(32), 2,
      sym_commented_child_line,
      aux_sym_commented_children_repeat1,
  [579] = 3,
    ACTIONS(138), 1,
      aux_sym_comment_content_token1,
    ACTIONS(142), 1,
      sym__newline,
    STATE(33), 2,
      sym_commented_child_line,
      aux_sym_commented_children_repeat1,
  [590] = 3,
    ACTIONS(138), 1,
      aux_sym_comment_content_token1,
    ACTIONS(142), 1,
      sym__newline,
    STATE(31), 2,
      sym_commented_child_line,
      aux_sym_commented_children_repeat1,
  [601] = 2,
    ACTIONS(156), 2,
      sym__newline,
      anon_sym_SPACE,
    ACTIONS(158), 2,
      sym_component_name,
      sym_identifier,
  [610] = 3,
    ACTIONS(138), 1,
      aux_sym_comment_content_token1,
    ACTIONS(142), 1,
      sym__newline,
    STATE(34), 2,
      sym_commented_child_line,
      aux_sym_commented_children_repeat1,
  [621] = 1,
    ACTIONS(160), 3,
      sym__dedent,
      sym__newline,
      aux_sym_comment_content_token1,
  [627] = 2,
    ACTIONS(164), 1,
      sym_identifier,
    ACTIONS(162), 2,
      sym__newline,
      anon_sym_SPACE,
  [635] = 1,
    ACTIONS(136), 3,
      sym__dedent,
      sym__newline,
      aux_sym_comment_content_token1,
  [641] = 3,
    ACTIONS(166), 1,
      aux_sym_comment_content_token1,
    ACTIONS(168), 1,
      sym__newline,
    STATE(58), 1,
      sym_comment_content,
  [651] = 2,
    ACTIONS(172), 1,
      sym_component_name,
    ACTIONS(170), 2,
      sym__newline,
      anon_sym_SPACE,
  [659] = 1,
    ACTIONS(174), 3,
      sym__newline,
      anon_sym_SPACE,
      sym_identifier,
  [665] = 3,
    ACTIONS(166), 1,
      aux_sym_comment_content_token1,
    ACTIONS(176), 1,
      sym__newline,
    STATE(62), 1,
      sym_comment_content,
  [675] = 1,
    ACTIONS(120), 3,
      sym__dedent,
      sym__newline,
      aux_sym_comment_content_token1,
  [681] = 1,
    ACTIONS(178), 2,
      sym__newline,
      anon_sym_SPACE,
  [686] = 1,
    ACTIONS(180), 2,
      sym__newline,
      anon_sym_SPACE,
  [691] = 1,
    ACTIONS(182), 2,
      sym__newline,
      anon_sym_SPACE,
  [696] = 1,
    ACTIONS(184), 2,
      sym__newline,
      anon_sym_SPACE,
  [701] = 2,
    ACTIONS(186), 1,
      aux_sym_localized_text_token1,
    STATE(48), 1,
      sym_localized_text,
  [708] = 1,
    ACTIONS(188), 2,
      sym__newline,
      anon_sym_SPACE,
  [713] = 1,
    ACTIONS(190), 2,
      sym__newline,
      anon_sym_SPACE,
  [718] = 2,
    ACTIONS(192), 1,
      anon_sym_SPACE,
    ACTIONS(194), 1,
      sym__newline,
  [725] = 1,
    ACTIONS(196), 2,
      sym__newline,
      anon_sym_SPACE,
  [730] = 1,
    ACTIONS(198), 2,
      sym__newline,
      anon_sym_SPACE,
  [735] = 1,
    ACTIONS(200), 2,
      sym__newline,
      anon_sym_SPACE,
  [740] = 1,
    ACTIONS(202), 1,
      sym__newline,
  [744] = 1,
    ACTIONS(204), 1,
      sym__newline,
  [748] = 1,
    ACTIONS(206), 1,
      sym__newline,
  [752] = 1,
    ACTIONS(208), 1,
      sym__newline,
  [756] = 1,
    ACTIONS(210), 1,
      sym__newline,
  [760] = 1,
    ACTIONS(212), 1,
      sym__newline,
  [764] = 1,
    ACTIONS(214), 1,
      sym__newline,
  [768] = 1,
    ACTIONS(216), 1,
      anon_sym_SPACE,
  [772] = 1,
    ACTIONS(218), 1,
      ts_builtin_sym_end,
};

static const uint32_t ts_small_parse_table_map[] = {
  [SMALL_STATE(10)] = 0,
  [SMALL_STATE(11)] = 31,
  [SMALL_STATE(12)] = 62,
  [SMALL_STATE(13)] = 93,
  [SMALL_STATE(14)] = 124,
  [SMALL_STATE(15)] = 155,
  [SMALL_STATE(16)] = 186,
  [SMALL_STATE(17)] = 211,
  [SMALL_STATE(18)] = 236,
  [SMALL_STATE(19)] = 261,
  [SMALL_STATE(20)] = 286,
  [SMALL_STATE(21)] = 311,
  [SMALL_STATE(22)] = 336,
  [SMALL_STATE(23)] = 361,
  [SMALL_STATE(24)] = 386,
  [SMALL_STATE(25)] = 411,
  [SMALL_STATE(26)] = 436,
  [SMALL_STATE(27)] = 461,
  [SMALL_STATE(28)] = 486,
  [SMALL_STATE(29)] = 499,
  [SMALL_STATE(30)] = 511,
  [SMALL_STATE(31)] = 523,
  [SMALL_STATE(32)] = 537,
  [SMALL_STATE(33)] = 551,
  [SMALL_STATE(34)] = 565,
  [SMALL_STATE(35)] = 579,
  [SMALL_STATE(36)] = 590,
  [SMALL_STATE(37)] = 601,
  [SMALL_STATE(38)] = 610,
  [SMALL_STATE(39)] = 621,
  [SMALL_STATE(40)] = 627,
  [SMALL_STATE(41)] = 635,
  [SMALL_STATE(42)] = 641,
  [SMALL_STATE(43)] = 651,
  [SMALL_STATE(44)] = 659,
  [SMALL_STATE(45)] = 665,
  [SMALL_STATE(46)] = 675,
  [SMALL_STATE(47)] = 681,
  [SMALL_STATE(48)] = 686,
  [SMALL_STATE(49)] = 691,
  [SMALL_STATE(50)] = 696,
  [SMALL_STATE(51)] = 701,
  [SMALL_STATE(52)] = 708,
  [SMALL_STATE(53)] = 713,
  [SMALL_STATE(54)] = 718,
  [SMALL_STATE(55)] = 725,
  [SMALL_STATE(56)] = 730,
  [SMALL_STATE(57)] = 735,
  [SMALL_STATE(58)] = 740,
  [SMALL_STATE(59)] = 744,
  [SMALL_STATE(60)] = 748,
  [SMALL_STATE(61)] = 752,
  [SMALL_STATE(62)] = 756,
  [SMALL_STATE(63)] = 760,
  [SMALL_STATE(64)] = 764,
  [SMALL_STATE(65)] = 768,
  [SMALL_STATE(66)] = 772,
};

static const TSParseActionEntry ts_parse_actions[] = {
  [0] = {.entry = {.count = 0, .reusable = false}},
  [1] = {.entry = {.count = 1, .reusable = false}}, RECOVER(),
  [3] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_source_file, 0, 0, 0),
  [5] = {.entry = {.count = 1, .reusable = true}}, SHIFT(54),
  [7] = {.entry = {.count = 1, .reusable = true}}, SHIFT(53),
  [9] = {.entry = {.count = 1, .reusable = false}}, SHIFT(53),
  [11] = {.entry = {.count = 1, .reusable = false}}, SHIFT(42),
  [13] = {.entry = {.count = 1, .reusable = true}}, SHIFT(37),
  [15] = {.entry = {.count = 1, .reusable = false}}, SHIFT(43),
  [17] = {.entry = {.count = 1, .reusable = true}}, SHIFT(43),
  [19] = {.entry = {.count = 1, .reusable = true}}, SHIFT(65),
  [21] = {.entry = {.count = 1, .reusable = true}}, SHIFT(52),
  [23] = {.entry = {.count = 1, .reusable = false}}, SHIFT(28),
  [25] = {.entry = {.count = 1, .reusable = true}}, SHIFT(57),
  [27] = {.entry = {.count = 1, .reusable = false}}, SHIFT(57),
  [29] = {.entry = {.count = 1, .reusable = false}}, SHIFT(55),
  [31] = {.entry = {.count = 1, .reusable = false}}, SHIFT(54),
  [33] = {.entry = {.count = 1, .reusable = true}}, REDUCE(aux_sym_source_file_repeat1, 2, 0, 0),
  [35] = {.entry = {.count = 2, .reusable = true}}, REDUCE(aux_sym_source_file_repeat1, 2, 0, 0), SHIFT_REPEAT(54),
  [38] = {.entry = {.count = 2, .reusable = true}}, REDUCE(aux_sym_source_file_repeat1, 2, 0, 0), SHIFT_REPEAT(53),
  [41] = {.entry = {.count = 2, .reusable = false}}, REDUCE(aux_sym_source_file_repeat1, 2, 0, 0), SHIFT_REPEAT(53),
  [44] = {.entry = {.count = 2, .reusable = false}}, REDUCE(aux_sym_source_file_repeat1, 2, 0, 0), SHIFT_REPEAT(42),
  [47] = {.entry = {.count = 2, .reusable = true}}, REDUCE(aux_sym_source_file_repeat1, 2, 0, 0), SHIFT_REPEAT(37),
  [50] = {.entry = {.count = 2, .reusable = false}}, REDUCE(aux_sym_source_file_repeat1, 2, 0, 0), SHIFT_REPEAT(43),
  [53] = {.entry = {.count = 2, .reusable = true}}, REDUCE(aux_sym_source_file_repeat1, 2, 0, 0), SHIFT_REPEAT(43),
  [56] = {.entry = {.count = 2, .reusable = true}}, REDUCE(aux_sym_source_file_repeat1, 2, 0, 0), SHIFT_REPEAT(65),
  [59] = {.entry = {.count = 2, .reusable = true}}, REDUCE(aux_sym_source_file_repeat1, 2, 0, 0), SHIFT_REPEAT(52),
  [62] = {.entry = {.count = 2, .reusable = false}}, REDUCE(aux_sym_source_file_repeat1, 2, 0, 0), SHIFT_REPEAT(28),
  [65] = {.entry = {.count = 2, .reusable = true}}, REDUCE(aux_sym_source_file_repeat1, 2, 0, 0), SHIFT_REPEAT(57),
  [68] = {.entry = {.count = 2, .reusable = false}}, REDUCE(aux_sym_source_file_repeat1, 2, 0, 0), SHIFT_REPEAT(57),
  [71] = {.entry = {.count = 2, .reusable = false}}, REDUCE(aux_sym_source_file_repeat1, 2, 0, 0), SHIFT_REPEAT(55),
  [74] = {.entry = {.count = 2, .reusable = false}}, REDUCE(aux_sym_source_file_repeat1, 2, 0, 0), SHIFT_REPEAT(54),
  [77] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_source_file, 1, 0, 0),
  [79] = {.entry = {.count = 1, .reusable = false}}, SHIFT(45),
  [81] = {.entry = {.count = 1, .reusable = true}}, SHIFT(22),
  [83] = {.entry = {.count = 2, .reusable = false}}, REDUCE(aux_sym_source_file_repeat1, 2, 0, 0), SHIFT_REPEAT(45),
  [86] = {.entry = {.count = 1, .reusable = true}}, SHIFT(26),
  [88] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_commented_line, 3, 0, 0),
  [90] = {.entry = {.count = 1, .reusable = false}}, REDUCE(sym_commented_line, 3, 0, 0),
  [92] = {.entry = {.count = 1, .reusable = true}}, SHIFT(36),
  [94] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_normal_line, 2, 0, 0),
  [96] = {.entry = {.count = 1, .reusable = false}}, REDUCE(sym_normal_line, 2, 0, 0),
  [98] = {.entry = {.count = 1, .reusable = true}}, SHIFT(7),
  [100] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_commented_line, 2, 0, 0),
  [102] = {.entry = {.count = 1, .reusable = false}}, REDUCE(sym_commented_line, 2, 0, 0),
  [104] = {.entry = {.count = 1, .reusable = true}}, SHIFT(8),
  [106] = {.entry = {.count = 1, .reusable = true}}, SHIFT(35),
  [108] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_line, 1, 0, 0),
  [110] = {.entry = {.count = 1, .reusable = false}}, REDUCE(sym_line, 1, 0, 0),
  [112] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_normal_line, 3, 0, 0),
  [114] = {.entry = {.count = 1, .reusable = false}}, REDUCE(sym_normal_line, 3, 0, 0),
  [116] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_commented_line, 4, 0, 0),
  [118] = {.entry = {.count = 1, .reusable = false}}, REDUCE(sym_commented_line, 4, 0, 0),
  [120] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_commented_children, 3, 0, 0),
  [122] = {.entry = {.count = 1, .reusable = false}}, REDUCE(sym_commented_children, 3, 0, 0),
  [124] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_children, 3, 0, 0),
  [126] = {.entry = {.count = 1, .reusable = false}}, REDUCE(sym_children, 3, 0, 0),
  [128] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym__token, 1, 0, 0),
  [130] = {.entry = {.count = 1, .reusable = true}}, SHIFT(44),
  [132] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_commented_child_line, 1, 0, 0),
  [134] = {.entry = {.count = 1, .reusable = true}}, SHIFT(38),
  [136] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_commented_child_line, 2, 0, 0),
  [138] = {.entry = {.count = 1, .reusable = true}}, SHIFT(63),
  [140] = {.entry = {.count = 1, .reusable = true}}, SHIFT(21),
  [142] = {.entry = {.count = 1, .reusable = true}}, SHIFT(29),
  [144] = {.entry = {.count = 2, .reusable = true}}, REDUCE(aux_sym_commented_children_repeat1, 2, 0, 0), SHIFT_REPEAT(63),
  [147] = {.entry = {.count = 1, .reusable = true}}, REDUCE(aux_sym_commented_children_repeat1, 2, 0, 0),
  [149] = {.entry = {.count = 2, .reusable = true}}, REDUCE(aux_sym_commented_children_repeat1, 2, 0, 0), SHIFT_REPEAT(29),
  [152] = {.entry = {.count = 1, .reusable = true}}, SHIFT(27),
  [154] = {.entry = {.count = 1, .reusable = true}}, SHIFT(46),
  [156] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_list_marker, 1, 0, 0),
  [158] = {.entry = {.count = 1, .reusable = true}}, SHIFT(50),
  [160] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_commented_child_line, 3, 0, 0),
  [162] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_identifier_with_modifier, 2, 0, 0),
  [164] = {.entry = {.count = 1, .reusable = true}}, SHIFT(56),
  [166] = {.entry = {.count = 1, .reusable = true}}, SHIFT(60),
  [168] = {.entry = {.count = 1, .reusable = true}}, SHIFT(12),
  [170] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_dict_marker, 1, 0, 0),
  [172] = {.entry = {.count = 1, .reusable = true}}, SHIFT(49),
  [174] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_property_modifier, 1, 0, 0),
  [176] = {.entry = {.count = 1, .reusable = true}}, SHIFT(14),
  [178] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_localized_text, 1, 0, 0),
  [180] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_localized_string, 3, 0, 0),
  [182] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_typed_dict, 2, 0, 0),
  [184] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_typed_list, 2, 0, 0),
  [186] = {.entry = {.count = 1, .reusable = true}}, SHIFT(47),
  [188] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_string_literal, 1, 0, 0),
  [190] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_binding_operator, 1, 0, 0),
  [192] = {.entry = {.count = 1, .reusable = true}}, SHIFT(9),
  [194] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_token_chain, 1, 0, 0),
  [196] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_boolean, 1, 0, 0),
  [198] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_identifier_with_modifier, 3, 0, 0),
  [200] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_number, 1, 0, 0),
  [202] = {.entry = {.count = 1, .reusable = true}}, SHIFT(10),
  [204] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_token_chain, 3, 0, 0),
  [206] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_comment_content, 1, 0, 0),
  [208] = {.entry = {.count = 1, .reusable = true}}, SHIFT(13),
  [210] = {.entry = {.count = 1, .reusable = true}}, SHIFT(15),
  [212] = {.entry = {.count = 1, .reusable = true}}, SHIFT(30),
  [214] = {.entry = {.count = 1, .reusable = true}}, SHIFT(11),
  [216] = {.entry = {.count = 1, .reusable = true}}, SHIFT(51),
  [218] = {.entry = {.count = 1, .reusable = true}},  ACCEPT_INPUT(),
};

enum ts_external_scanner_symbol_identifiers {
  ts_external_token__indent = 0,
  ts_external_token__dedent = 1,
  ts_external_token__newline = 2,
};

static const TSSymbol ts_external_scanner_symbol_map[EXTERNAL_TOKEN_COUNT] = {
  [ts_external_token__indent] = sym__indent,
  [ts_external_token__dedent] = sym__dedent,
  [ts_external_token__newline] = sym__newline,
};

static const bool ts_external_scanner_states[7][EXTERNAL_TOKEN_COUNT] = {
  [1] = {
    [ts_external_token__indent] = true,
    [ts_external_token__dedent] = true,
    [ts_external_token__newline] = true,
  },
  [2] = {
    [ts_external_token__dedent] = true,
  },
  [3] = {
    [ts_external_token__indent] = true,
  },
  [4] = {
    [ts_external_token__indent] = true,
    [ts_external_token__dedent] = true,
  },
  [5] = {
    [ts_external_token__newline] = true,
  },
  [6] = {
    [ts_external_token__dedent] = true,
    [ts_external_token__newline] = true,
  },
};

#ifdef __cplusplus
extern "C" {
#endif
void *tree_sitter_viewtree_external_scanner_create(void);
void tree_sitter_viewtree_external_scanner_destroy(void *);
bool tree_sitter_viewtree_external_scanner_scan(void *, TSLexer *, const bool *);
unsigned tree_sitter_viewtree_external_scanner_serialize(void *, char *);
void tree_sitter_viewtree_external_scanner_deserialize(void *, const char *, unsigned);

#ifdef TREE_SITTER_HIDE_SYMBOLS
#define TS_PUBLIC
#elif defined(_WIN32)
#define TS_PUBLIC __declspec(dllexport)
#else
#define TS_PUBLIC __attribute__((visibility("default")))
#endif

TS_PUBLIC const TSLanguage *tree_sitter_viewtree(void) {
  static const TSLanguage language = {
    .abi_version = LANGUAGE_VERSION,
    .symbol_count = SYMBOL_COUNT,
    .alias_count = ALIAS_COUNT,
    .token_count = TOKEN_COUNT,
    .external_token_count = EXTERNAL_TOKEN_COUNT,
    .state_count = STATE_COUNT,
    .large_state_count = LARGE_STATE_COUNT,
    .production_id_count = PRODUCTION_ID_COUNT,
    .supertype_count = SUPERTYPE_COUNT,
    .field_count = FIELD_COUNT,
    .max_alias_sequence_length = MAX_ALIAS_SEQUENCE_LENGTH,
    .parse_table = &ts_parse_table[0][0],
    .small_parse_table = ts_small_parse_table,
    .small_parse_table_map = ts_small_parse_table_map,
    .parse_actions = ts_parse_actions,
    .symbol_names = ts_symbol_names,
    .symbol_metadata = ts_symbol_metadata,
    .public_symbol_map = ts_symbol_map,
    .alias_map = ts_non_terminal_alias_map,
    .alias_sequences = &ts_alias_sequences[0][0],
    .lex_modes = (const void*)ts_lex_modes,
    .lex_fn = ts_lex,
    .external_scanner = {
      &ts_external_scanner_states[0][0],
      ts_external_scanner_symbol_map,
      tree_sitter_viewtree_external_scanner_create,
      tree_sitter_viewtree_external_scanner_destroy,
      tree_sitter_viewtree_external_scanner_scan,
      tree_sitter_viewtree_external_scanner_serialize,
      tree_sitter_viewtree_external_scanner_deserialize,
    },
    .primary_state_ids = ts_primary_state_ids,
    .name = "viewtree",
    .max_reserved_word_set_size = 0,
    .metadata = {
      .major_version = 0,
      .minor_version = 1,
      .patch_version = 0,
    },
  };
  return &language;
}
#ifdef __cplusplus
}
#endif
